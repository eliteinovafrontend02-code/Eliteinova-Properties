...
const ColdStorage = () => {
  ...
  const normalizeProperties = useCallback((list) => {
    ...
    return list.map(p => ({
      ...p,
      propertyType: COLD_STORAGE_TYPE.label
    }));
  }, []);
  ...
  const generateMockProperties = useCallback(() => {
    const propertyTitles = [
      'ColdChain Logistics Hub', 'Galaxy Cold Storage Facility', 'Riverside Logistics Hub',
      'Silverline Cold Storage Unit', 'Metro Logistics Hub', 'Crown Cold Storage Facility',
      'Sunrise Logistics Hub', 'Union Square Cold Storage Unit', 'Palm Grove Logistics Hub',
      'Fortune Cold Storage Facility', 'Emerald Logistics Hub', 'Pinnacle Cold Storage Unit',
      'Aster Logistics Hub', 'Zenith Cold Storage Facility', 'Orion Logistics Hub',
      'Regal Cold Storage Unit', 'Meridian Logistics Hub', 'Phoenix Cold Storage Facility'
    ];
    ...
    for (let i = 1; i <= 40; i++) {
      ...
      propertiesList.push({
        id: `cs_${i}`,
        propertyId: `CS-${String(i).padStart(4, '0')}`,
        propertyTitle: propertyTitles[Math.floor(Math.random() * propertyTitles.length)],
        propertyType: COLD_STORAGE_TYPE.label,
        listingType,
        description: `A well-located cold storage / logistics hub with temperature-controlled storage and easy highway access, available for ${listingType.toLowerCase()}.`,
