...
const Showroom = () => {
  ...
  const normalizeProperties = useCallback((list) => {
    ...
    return list.map(p => ({
      ...p,
      propertyType: SHOWROOM_TYPE.label
    }));
  }, []);
  ...
  const generateMockProperties = useCallback(() => {
    const propertyTitles = [
      'Grand Motors Showroom', 'Highway Auto Showroom', 'Galaxy Furniture Showroom',
      'Riverside Appliance Showroom', 'Junction Road Bike Showroom', 'Silverline Car Showroom',
      'Metro Plaza Electronics Showroom', 'Crown Interiors Showroom', 'Sunrise Auto Showroom',
      'Union Square Furniture Showroom', 'Palm Grove Bike Showroom', 'Fortune Motors Showroom',
      'Emerald Appliance Showroom', 'Pinnacle Car Showroom', 'Aster Electronics Showroom',
      'Zenith Interiors Showroom', 'Orion Auto Showroom', 'Regal Furniture Showroom'
    ];
    ...
    for (let i = 1; i <= 40; i++) {
      ...
      propertiesList.push({
        id: `sr_${i}`,
        propertyId: `SR-${String(i).padStart(4, '0')}`,
        propertyTitle: propertyTitles[Math.floor(Math.random() * propertyTitles.length)],
        propertyType: SHOWROOM_TYPE.label,
        listingType,
        description: `A well-located showroom with expansive display space and strong brand visibility, available for ${listingType.toLowerCase()}.`,
