...
const MixedUse = () => {
  ...
  const normalizeProperties = useCallback((list) => {
    ...
    return list.map(p => ({
      ...p,
      propertyType: MIXED_USE_TYPE.label
    }));
  }, []);
  ...
  const generateMockProperties = useCallback(() => {
    const propertyTitles = [
      'Union Square Mixed-use Property', 'Galaxy Mixed-use Complex', 'Riverside Mixed-use Property',
      'Silverline Mixed-use Complex', 'Metro Mixed-use Property', 'Crown Mixed-use Complex',
      'Sunrise Mixed-use Property', 'Palm Grove Mixed-use Complex', 'Fortune Mixed-use Property',
      'Emerald Mixed-use Complex', 'Pinnacle Mixed-use Property', 'Aster Mixed-use Complex',
      'Zenith Mixed-use Property', 'Orion Mixed-use Complex', 'Regal Mixed-use Property',
      'Meridian Mixed-use Complex', 'Phoenix Mixed-use Property', 'Cyber Mixed-use Complex'
    ];
    ...
    for (let i = 1; i <= 40; i++) {
      ...
      propertiesList.push({
        ...
        propertyType: MIXED_USE_TYPE.label,
        listingType,
        description: `A well-located mixed-use commercial property combining retail, office and residential-friendly floors, available for ${listingType.toLowerCase()}.`,
