...
const ShoppingMallSpace = () => {
  ...
  const normalizeProperties = useCallback((list) => {
    ...
    return list.map(p => ({
      ...p,
      propertyType: SHOPPING_MALL_SPACE_TYPE.label
    }));
  }, []);
  ...
  const generateMockProperties = useCallback(() => {
    const propertyTitles = [
      'Phoenix Mall Retail Unit', 'Galaxy Mall Shopping Space', 'Crown Mall Retail Floor',
      'Riverside Mall Shopping Unit', 'Junction Mall Retail Space', 'Silverline Mall Shopping Floor',
      'Metro Mall Retail Unit', 'Sunrise Mall Shopping Space', 'Union Square Mall Retail Floor',
      'Palm Grove Mall Shopping Unit', 'Fortune Mall Retail Space', 'Emerald Mall Shopping Floor',
      'Pinnacle Mall Retail Unit', 'Aster Mall Shopping Space', 'Zenith Mall Retail Floor',
      'Orion Mall Shopping Unit', 'Regal Mall Retail Space', 'Meridian Mall Shopping Floor'
    ];
    ...
    for (let i = 1; i <= 40; i++) {
      ...
      propertiesList.push({
        id: `sm_${i}`,
        propertyId: `SM-${String(i).padStart(4, '0')}`,
        propertyTitle: propertyTitles[Math.floor(Math.random() * propertyTitles.length)],
        propertyType: SHOPPING_MALL_SPACE_TYPE.label,
        listingType,
        description: `A well-located shopping mall space with high footfall and premium retail visibility, available for ${listingType.toLowerCase()}.`,
