...
const ClinicSpace = () => {
  ...
  const normalizeProperties = useCallback((list) => {
    ...
    return list.map(p => ({
      ...p,
      propertyType: CLINIC_SPACE_TYPE.label
    }));
  }, []);
  ...
  const generateMockProperties = useCallback(() => {
    const propertyTitles = [
      'Wellness Clinic Space', 'Galaxy Hospital Space', 'Riverside Clinic Unit',
      'Silverline Hospital Space', 'Metro Health Clinic Space', 'Crown Hospital Unit',
      'Sunrise Clinic Space', 'Union Square Hospital Space', 'Palm Grove Clinic Unit',
      'Fortune Hospital Space', 'Emerald Clinic Space', 'Pinnacle Hospital Unit',
      'Aster Clinic Space', 'Zenith Hospital Space', 'Orion Clinic Unit',
      'Regal Hospital Space', 'Meridian Clinic Space', 'Phoenix Hospital Unit'
    ];
    ...
    for (let i = 1; i <= 40; i++) {
      ...
      propertiesList.push({
        id: `ch_${i}`,
        propertyId: `CH-${String(i).padStart(4, '0')}`,
        propertyTitle: propertyTitles[Math.floor(Math.random() * propertyTitles.length)],
        propertyType: CLINIC_SPACE_TYPE.label,
        listingType,
        description: `A well-located clinic / hospital space with dedicated patient care infrastructure and easy accessibility, available for ${listingType.toLowerCase()}.`,
