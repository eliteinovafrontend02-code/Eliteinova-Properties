...
const Multiplex = () => {
  ...
  const normalizeProperties = useCallback((list) => {
    ...
    return list.map(p => ({
      ...p,
      propertyType: MULTIPLEX_TYPE.label
    }));
  }, []);
  ...
  const generateMockProperties = useCallback(() => {
    const propertyTitles = [
      'Galaxy Multiplex Entertainment Unit', 'Silverline Multiplex Space', 'Riverside Entertainment Zone',
      'Metro Multiplex Unit', 'Crown Entertainment Space', 'Sunrise Multiplex Zone',
      'Union Square Entertainment Unit', 'Palm Grove Multiplex Space', 'Fortune Entertainment Zone',
      'Emerald Multiplex Unit', 'Pinnacle Entertainment Space', 'Aster Multiplex Zone',
      'Zenith Entertainment Unit', 'Orion Multiplex Space', 'Regal Entertainment Zone',
      'Meridian Multiplex Unit', 'Phoenix Entertainment Space', 'Cyber Multiplex Zone'
    ];
    ...
    for (let i = 1; i <= 40; i++) {
      ...
      propertiesList.push({
        id: `mx_${i}`,
        propertyId: `MX-${String(i).padStart(4, '0')}`,
        propertyTitle: propertyTitles[Math.floor(Math.random() * propertyTitles.length)],
        propertyType: MULTIPLEX_TYPE.label,
        listingType,
        description: `A well-located multiplex / entertainment space with premium screening facilities and strong footfall, available for ${listingType.toLowerCase()}.`,
