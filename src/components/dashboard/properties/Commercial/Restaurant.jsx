...
const Restaurant = () => {
  ...
  const normalizeProperties = useCallback((list) => {
    ...
    return list.map(p => ({
      ...p,
      propertyType: RESTAURANT_TYPE.label
    }));
  }, []);
  ...
  const generateMockProperties = useCallback(() => {
    const propertyTitles = [
      'Spice Route Restaurant Space', 'Galaxy Cafe Space', 'Riverside Restaurant Unit',
      'Junction Corner Cafe Space', 'Silverline Restaurant Space', 'Metro Plaza Cafe Unit',
      'Crown Restaurant Space', 'Sunrise Cafe Corner', 'Union Square Restaurant Space',
      'Palm Grove Cafe Space', 'Fortune Restaurant Unit', 'Emerald Cafe Space',
      'Pinnacle Restaurant Space', 'Aster Cafe Corner', 'Zenith Restaurant Unit',
      'Orion Cafe Space', 'Regal Restaurant Space', 'Meridian Cafe Unit'
    ];
    ...
    for (let i = 1; i <= 40; i++) {
      ...
      propertiesList.push({
        id: `rc_${i}`,
        propertyId: `RC-${String(i).padStart(4, '0')}`,
        propertyTitle: propertyTitles[Math.floor(Math.random() * propertyTitles.length)],
        propertyType: RESTAURANT_TYPE.label,
        listingType,
        description: `A well-located restaurant / cafe space with dedicated kitchen setup and strong dining footfall, available for ${listingType.toLowerCase()}.`,
