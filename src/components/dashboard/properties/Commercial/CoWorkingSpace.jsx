...
const CoWorkingSpace = () => {
  ...
  const normalizeProperties = useCallback((list) => {
    ...
    return list.map(p => ({
      ...p,
      propertyType: COWORKING_SPACE_TYPE.label
    }));
  }, []);
  ...
  const generateMockProperties = useCallback(() => {
    const propertyTitles = [
      'Innov8 Co-working Hub', 'WorkNest Shared Office Space', 'Galaxy Co-working Studio',
      'Riverside Collaborative Workspace', 'Junction Point Co-working Hub', 'Silverline Shared Workspace',
      'Metro Plaza Co-working Studio', 'Crown Collaborative Hub', 'Sunrise Co-working Space',
      'Union Square Shared Office', 'Palm Grove Co-working Hub', 'Fortune Collaborative Workspace',
      'Emerald Co-working Studio', 'Pinnacle Shared Workspace', 'Aster Co-working Hub',
      'Zenith Collaborative Space', 'Orion Co-working Studio', 'Regal Shared Office Hub'
    ];
    ...
    for (let i = 1; i <= 40; i++) {
      ...
      propertiesList.push({
        id: `cw_${i}`,
        propertyId: `CW-${String(i).padStart(4, '0')}`,
        propertyTitle: propertyTitles[Math.floor(Math.random() * propertyTitles.length)],
        propertyType: COWORKING_SPACE_TYPE.label,
        listingType,
        description: `A well-located co-working space with flexible seating and modern shared amenities, available for ${listingType.toLowerCase()}.`,
