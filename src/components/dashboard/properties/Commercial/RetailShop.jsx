...
const RetailShop = () => {
  ...
  const normalizeProperties = useCallback((list) => {
    ...
    return list.map(p => ({
      ...p,
      propertyType: RETAIL_SHOP_TYPE.label
    }));
  }, []);
  ...
  const generateMockProperties = useCallback(() => {
    const propertyTitles = [
      'Marina Street Retail Shop', 'High Street Corner Shop', 'Galaxy Mall Retail Unit',
      'Riverside Retail Outlet', 'Junction Road Retail Shop', 'Silverline Retail Store',
      'Metro Plaza Retail Shop', 'Crown Market Retail Unit', 'Sunrise Arcade Retail Shop',
      'Union Square Retail Store', 'Palm Grove Retail Outlet', 'Fortune Retail Shop',
      'Emerald Bazaar Retail Unit', 'Pinnacle Retail Store', 'Aster Corner Retail Shop',
      'Zenith Market Retail Outlet', 'Orion Plaza Retail Shop', 'Regal Street Retail Store'
    ];
    ...
    for (let i = 1; i <= 40; i++) {
      ...
      propertiesList.push({
        id: `rs_${i}`,
        propertyId: `RS-${String(i).padStart(4, '0')}`,
        propertyTitle: propertyTitles[Math.floor(Math.random() * propertyTitles.length)],
        propertyType: RETAIL_SHOP_TYPE.label,
        listingType,
        description: `A well-located retail shop with excellent footfall and strong street visibility, available for ${listingType.toLowerCase()}.`,
