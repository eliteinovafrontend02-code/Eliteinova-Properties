...
const Warehouse = () => {
  ...
  const normalizeProperties = useCallback((list) => {
    ...
    return list.map(p => ({
      ...p,
      propertyType: WAREHOUSE_TYPE.label
    }));
  }, []);
  ...
  const generateMockProperties = useCallback(() => {
    const propertyTitles = [
      'Prestige Logistics Warehouse', 'Meridian Industrial Warehouse', 'Skyline Storage Warehouse',
      'Cyber Gateway Warehouse', 'Horizon Distribution Center', 'Silverline Warehouse',
      'Innov8 Logistics Hub', 'Regal Industrial Warehouse', 'Metro Distribution Warehouse',
      'Union Square Warehouse Complex', 'Galaxy Tech Warehouse', 'Fortune Warehouse Block',
      'Crown Logistics Warehouse', 'Emerald Industrial Warehouse', 'Pinnacle Distribution Center',
      'Aster Warehouse Chambers', 'Zenith Logistics Warehouse', 'Orion Industrial Warehouse'
    ];
    ...
    const streets = ['Warehouse Avenue', 'MG Road', 'Ring Road', 'Commerce Street', 'Tech Park Lane', 'Junction Road', 'Market Street', 'Corporate Boulevard'];
    ...
    for (let i = 1; i <= 40; i++) {
      ...
      propertiesList.push({
        id: `wh_${i}`,
        propertyId: `WH-${String(i).padStart(4, '0')}`,
        propertyTitle: propertyTitles[Math.floor(Math.random() * propertyTitles.length)],
        propertyType: WAREHOUSE_TYPE.label,
        listingType,
        description: `A well-located warehouse with modern infrastructure and strong connectivity, available for ${listingType.toLowerCase()}.`,
