...
const PetrolBunk = () => {
  ...
  const normalizeProperties = useCallback((list) => {
    ...
    return list.map(p => ({
      ...p,
      propertyType: PETROL_BUNK_TYPE.label
    }));
  }, []);
  ...
  const generateMockProperties = useCallback(() => {
    const propertyTitles = [
      'Highway Petrol Bunk Outlet', 'Galaxy Fuel Station', 'Riverside Petrol Bunk',
      'Silverline Fuel Station', 'Metro Highway Petrol Bunk', 'Crown Fuel Station',
      'Sunrise Petrol Bunk Outlet', 'Union Square Fuel Station', 'Palm Grove Petrol Bunk',
      'Fortune Fuel Station', 'Emerald Petrol Bunk Outlet', 'Pinnacle Fuel Station',
      'Aster Petrol Bunk', 'Zenith Fuel Station', 'Orion Petrol Bunk Outlet',
      'Regal Fuel Station', 'Meridian Petrol Bunk', 'Phoenix Fuel Station'
    ];
    ...
    for (let i = 1; i <= 40; i++) {
      ...
      propertiesList.push({
        id: `pb_${i}`,
        propertyId: `PB-${String(i).padStart(4, '0')}`,
        propertyTitle: propertyTitles[Math.floor(Math.random() * propertyTitles.length)],
        propertyType: PETROL_BUNK_TYPE.label,
        listingType,
        description: `A well-located petrol bunk / fuel station with highway frontage and strong vehicle traffic, available for ${listingType.toLowerCase()}.`,
