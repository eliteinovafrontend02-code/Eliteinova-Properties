...
const EducationalInstitution = () => {
  ...
  const normalizeProperties = useCallback((list) => {
    ...
    return list.map(p => ({
      ...p,
      propertyType: EDUCATIONAL_INSTITUTION_TYPE.label
    }));
  }, []);
  ...
  const generateMockProperties = useCallback(() => {
    const propertyTitles = [
      'Bright Future Institution Campus', 'Galaxy Education Campus', 'Riverside Institution Property',
      'Silverline Education Campus', 'Metro Learning Institution Property', 'Crown Education Campus',
      'Sunrise Institution Property', 'Union Square Education Campus', 'Palm Grove Institution Property',
      'Fortune Education Campus', 'Emerald Institution Property', 'Pinnacle Education Campus',
      'Aster Institution Property', 'Zenith Education Campus', 'Orion Institution Property',
      'Regal Education Campus', 'Meridian Institution Property', 'Phoenix Education Campus'
    ];
    ...
    for (let i = 1; i <= 40; i++) {
      ...
      propertiesList.push({
        id: `ei_${i}`,
        propertyId: `EI-${String(i).padStart(4, '0')}`,
        propertyTitle: propertyTitles[Math.floor(Math.random() * propertyTitles.length)],
        propertyType: EDUCATIONAL_INSTITUTION_TYPE.label,
        listingType,
        description: `A well-located educational institution property with ample classroom space and strong campus infrastructure, available for ${listingType.toLowerCase()}.`,
