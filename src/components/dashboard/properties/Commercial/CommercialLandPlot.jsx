...
const CommercialLandPlot = () => {
  ...
  const normalizeProperties = useCallback((list) => {
    ...
    return list.map(p => ({
      ...p,
      propertyType: COMMERCIAL_LAND_PLOT_TYPE.label
    }));
  }, []);
  ...
  const generateMockProperties = useCallback(() => {
    const propertyTitles = [
      'Prestige Commercial Land Plot', 'Meridian Business Land Parcel', 'Skyline Commercial Plot',
      'Cyber Gateway Land Parcel', 'Horizon Commercial Land', 'Silverline Land Plot',
      'Innov8 Commercial Land', 'Regal Business Land Plot', 'Metro Commercial Parcel',
      'Union Square Land Plot', 'Galaxy Tech Land Parcel', 'Fortune Commercial Land',
      'Crown Land Plot', 'Emerald Commercial Parcel', 'Pinnacle Business Land',
      'Aster Land Plot', 'Zenith Commercial Land', 'Orion Business Parcel'
    ];
    ...
    const areas = ['Commercial District', 'Whitefield', 'Hitech City', 'Andheri', 'Nehru Place', 'Sector V', 'SG Highway', 'Infopark'];
    const streets = ['Commercial Avenue', 'MG Road', 'Ring Road', 'Business Street', 'Tech Park Lane', 'Junction Road', 'Market Street', 'Corporate Boulevard'];
    ...
    for (let i = 1; i <= 40; i++) {
      ...
      propertiesList.push({
        id: `clp_${i}`,
        propertyId: `CLP-${String(i).padStart(4, '0')}`,
        propertyTitle: propertyTitles[Math.floor(Math.random() * propertyTitles.length)],
        propertyType: COMMERCIAL_LAND_PLOT_TYPE.label,
        listingType,
        description: `A prime commercial land plot with excellent connectivity and development potential, available for ${listingType.toLowerCase()}.`,
