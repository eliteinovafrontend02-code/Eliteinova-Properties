...
const BusinessCenter = () => {
  ...
  const normalizeProperties = useCallback((list) => {
    ...
    return list.map(p => ({
      ...p,
      propertyType: BUSINESS_CENTER_TYPE.label
    }));
  }, []);
  ...
  const generateMockProperties = useCallback(() => {
    const propertyTitles = [
      'Meridian Business Center', 'Prestige Executive Business Center', 'Galaxy Corporate Business Center',
      'Riverside Business Center', 'Junction Hub Business Center', 'Silverline Business Center',
      'Metro Plaza Business Center', 'Crown Executive Business Center', 'Sunrise Business Center',
      'Union Square Business Center', 'Palm Grove Business Center', 'Fortune Corporate Business Center',
      'Emerald Business Center', 'Pinnacle Executive Business Center', 'Aster Business Center',
      'Zenith Corporate Business Center', 'Orion Business Center', 'Regal Executive Business Center'
    ];
    ...
    for (let i = 1; i <= 40; i++) {
      ...
      propertiesList.push({
        id: `bc_${i}`,
        propertyId: `BC-${String(i).padStart(4, '0')}`,
        propertyTitle: propertyTitles[Math.floor(Math.random() * propertyTitles.length)],
        propertyType: BUSINESS_CENTER_TYPE.label,
        listingType,
        description: `A well-located business center with fully serviced offices and premium meeting facilities, available for ${listingType.toLowerCase()}.`,
