...
const CommercialComplex = () => {
  ...
  const normalizeProperties = useCallback((list) => {
    ...
    return list.map(p => ({
      ...p,
      propertyType: COMMERCIAL_COMPLEX_TYPE.label
    }));
  }, []);
  ...
  const generateMockProperties = useCallback(() => {
    const propertyTitles = [
      'Silverline Commercial Complex', 'Meridian Commercial Complex', 'Galaxy Business Complex',
      'Riverside Commercial Complex', 'Junction Hub Commercial Complex', 'Crown Commercial Complex',
      'Metro Plaza Commercial Complex', 'Sunrise Commercial Complex', 'Union Square Commercial Complex',
      'Palm Grove Commercial Complex', 'Fortune Business Complex', 'Emerald Commercial Complex',
      'Pinnacle Commercial Complex', 'Aster Business Complex', 'Zenith Commercial Complex',
      'Orion Business Complex', 'Regal Commercial Complex', 'Phoenix Business Complex'
    ];
    ...
    for (let i = 1; i <= 40; i++) {
      ...
      propertiesList.push({
        id: `cc_${i}`,
        propertyId: `CC-${String(i).padStart(4, '0')}`,
        propertyTitle: propertyTitles[Math.floor(Math.random() * propertyTitles.length)],
        propertyType: COMMERCIAL_COMPLEX_TYPE.label,
        listingType,
        description: `A well-located commercial complex with multi-use floors and strong business infrastructure, available for ${listingType.toLowerCase()}.`,
