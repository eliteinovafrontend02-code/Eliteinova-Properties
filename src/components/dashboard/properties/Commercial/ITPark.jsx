...
const ITPark = () => {
  ...
  const normalizeProperties = useCallback((list) => {
    ...
    return list.map(p => ({
      ...p,
      propertyType: IT_PARK_TYPE.label
    }));
  }, []);
  ...
  const generateMockProperties = useCallback(() => {
    const propertyTitles = [
      'Cyber Tower Tech Park Space', 'Galaxy IT Park Space', 'Riverside Tech Park Floor',
      'Silverline IT Park Space', 'Metro Tech Park Floor', 'Crown IT Park Space',
      'Sunrise Tech Park Floor', 'Union Square IT Park Space', 'Palm Grove Tech Park Floor',
      'Fortune IT Park Space', 'Emerald Tech Park Floor', 'Pinnacle IT Park Space',
      'Aster Tech Park Floor', 'Zenith IT Park Space', 'Orion Tech Park Floor',
      'Regal IT Park Space', 'Meridian Tech Park Floor', 'Phoenix IT Park Space'
    ];
    ...
    for (let i = 1; i <= 40; i++) {
      ...
      propertiesList.push({
        id: `itp_${i}`,
        propertyId: `ITP-${String(i).padStart(4, '0')}`,
        propertyTitle: propertyTitles[Math.floor(Math.random() * propertyTitles.length)],
        propertyType: IT_PARK_TYPE.label,
        listingType,
        description: `A well-located IT park / tech park space with high-speed connectivity and modern tech infrastructure, available for ${listingType.toLowerCase()}.`,
