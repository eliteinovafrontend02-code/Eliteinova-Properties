...
const IndustrialProperty = () => {
  ...
  const normalizeProperties = useCallback((list) => {
    ...
    return list.map(p => ({
      ...p,
      propertyType: INDUSTRIAL_TYPE.label
    }));
  }, []);
  ...
  const generateMockProperties = useCallback(() => {
    const propertyTitles = [
      'Prestige Industrial Estate', 'Meridian Industrial Park', 'Skyline Industrial Zone',
      'Cyber Gateway Industrial Area', 'Horizon Manufacturing Hub', 'Silverline Industrial Complex',
      'Innov8 Industrial Estate', 'Regal Industrial Park', 'Metro Manufacturing Zone',
      'Union Square Industrial Area', 'Galaxy Tech Industrial Hub', 'Fortune Industrial Block',
      'Crown Industrial Estate', 'Emerald Industrial Park', 'Pinnacle Manufacturing Zone',
      'Aster Industrial Complex', 'Zenith Industrial Hub', 'Orion Industrial Area'
    ];
    ...
    const areas = ['Industrial Estate', 'Whitefield', 'Hitech City', 'Andheri', 'Nehru Place', 'Sector V', 'SG Highway', 'Infopark'];
    const streets = ['Industrial Avenue', 'MG Road', 'Ring Road', 'Commerce Street', 'Tech Park Lane', 'Junction Road', 'Market Street', 'Corporate Boulevard'];
    ...
    for (let i = 1; i <= 40; i++) {
      ...
      propertiesList.push({
        id: `ip_${i}`,
        propertyId: `IP-${String(i).padStart(4, '0')}`,
        propertyTitle: propertyTitles[Math.floor(Math.random() * propertyTitles.length)],
        propertyType: INDUSTRIAL_TYPE.label,
        listingType,
        description: `A well-located industrial property with modern infrastructure and excellent connectivity, available for ${listingType.toLowerCase()}.`,
