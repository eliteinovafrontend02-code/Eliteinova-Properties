...
const CondominiumApartment = () => {
  ...
  const normalizeProperties = useCallback((list) => {
    ...
    return list.map(p => ({
      ...p,
      propertyType: CONDOMINIUM_APARTMENT_TYPE.label
    }));
  }, []);
  ...
  const generateMockProperties = useCallback(() => {
    const propertyTitles = [
      'Skyline Condominium', 'Metro Heights Condominium', 'Palm Grove Condominium Residency',
      'Green Valley Condominium', 'Lake View Condominium Tower', 'Royal Palm Condominium',
      'Silver Oak Condominium', 'Golden Meadows Condominium Complex', 'Cedar Woods Condominium',
      'Maple Leaf Condominium Tower', 'Orchid Garden Condominium Suites', 'Tulip Condominium',
      'Lotus Condominium Tower', 'Jasmine Condominium Flats', 'Emerald Greens Condominium',
      'Pearl Residency Condominium', 'Ruby Enclave Condominium', 'Sapphire Condominium Tower'
    ];
    ...
    for (let i = 1; i <= 40; i++) {
      ...
      propertiesList.push({
        id: `cd_${i}`,
        propertyId: `CD-${String(i).padStart(4, '0')}`,
        propertyTitle: propertyTitles[Math.floor(Math.random() * propertyTitles.length)],
        propertyType: CONDOMINIUM_APARTMENT_TYPE.label,
        listingType,
        description: `A premium condominium apartment with exclusive amenities and community living, available for ${listingType.toLowerCase()}.`,
