...
const PenthouseApartment = () => {
  ...
  const normalizeProperties = useCallback((list) => {
    ...
    return list.map(p => ({
      ...p,
      propertyType: PENTHOUSE_APARTMENT_TYPE.label
    }));
  }, []);
  ...
  const generateMockProperties = useCallback(() => {
    const propertyTitles = [
      'Cloud Nine Penthouse', 'Azure Sky Penthouse', 'Royal Penthouse Suite',
      'Golden Crest Penthouse', 'Platinum Penthouse Tower', 'Diamond Penthouse Residency',
      'Imperial Penthouse', 'Majestic Penthouse Suite', 'Grand Penthouse Tower',
      'Elite Penthouse Residency', 'Prestige Penthouse', 'Opulent Penthouse Suite',
      'Noble Penthouse Tower', 'Regency Penthouse', 'Sovereign Penthouse Residency',
      'Victoria Penthouse Suite', 'Windsor Penthouse Tower', 'Buckingham Penthouse'
    ];
    ...
    for (let i = 1; i <= 40; i++) {
      ...
      propertiesList.push({
        id: `ph_${i}`,
        propertyId: `PH-${String(i).padStart(4, '0')}`,
        propertyTitle: propertyTitles[Math.floor(Math.random() * propertyTitles.length)],
        propertyType: PENTHOUSE_APARTMENT_TYPE.label,
        listingType,
        description: `A stunning penthouse apartment with panoramic views and luxurious finishes, available for ${listingType.toLowerCase()}.`,
