...
const GatedCommunityApartment = () => {
  ...
  const normalizeProperties = useCallback((list) => {
    ...
    return list.map(p => ({
      ...p,
      propertyType: GATED_COMMUNITY_TYPE.label
    }));
  }, []);
  ...
  const generateMockProperties = useCallback(() => {
    const propertyTitles = [
      'Golden Gate Gated Community', 'Fortune Gated Enclave', 'Palm Grove Gated Residency',
      'Green Valley Gated Complex', 'Lake View Gated Community', 'Royal Palm Gated Enclave',
      'Silver Oak Gated Residency', 'Golden Meadows Gated Community', 'Cedar Woods Gated Enclave',
      'Maple Leaf Gated Complex', 'Orchid Garden Gated Residency', 'Tulip Gated Community',
      'Lotus Gated Enclave', 'Jasmine Gated Residency', 'Emerald Greens Gated Complex',
      'Pearl Residency Gated Community', 'Ruby Enclave Gated', 'Sapphire Gated Tower'
    ];
    ...
    for (let i = 1; i <= 40; i++) {
      ...
      propertiesList.push({
        id: `gc_${i}`,
        propertyId: `GC-${String(i).padStart(4, '0')}`,
        propertyTitle: propertyTitles[Math.floor(Math.random() * propertyTitles.length)],
        propertyType: GATED_COMMUNITY_TYPE.label,
        listingType,
        description: `A premium gated community apartment with 24/7 security and excellent amenities, available for ${listingType.toLowerCase()}.`,
