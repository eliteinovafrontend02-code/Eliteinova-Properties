...
const StudioApartment = () => {
  ...
  const normalizeProperties = useCallback((list) => {
    ...
    return list.map(p => ({
      ...p,
      propertyType: STUDIO_APARTMENT_TYPE.label
    }));
  }, []);
  ...
  const generateMockProperties = useCallback(() => {
    const propertyTitles = [
      'Cozy Nest Studio', 'Compact Living Studio', 'Urban Studio Apartment',
      'Minimalist Studio', 'Modern Studio Suite', 'City View Studio',
      'Garden Studio Apartment', 'Luxury Studio Residency', 'Palm Grove Studio',
      'Lake View Studio', 'Royal Studio Suite', 'Silver Studio Apartment',
      'Golden Studio Residency', 'Cedar Studio Complex', 'Maple Studio Apartment',
      'Orchid Studio Suites', 'Tulip Studio', 'Lotus Studio Tower',
      'Jasmine Studio Flats', 'Emerald Studio Residency'
    ];
    ...
    for (let i = 1; i <= 40; i++) {
      ...
      propertiesList.push({
        id: `st_${i}`,
        propertyId: `ST-${String(i).padStart(4, '0')}`,
        propertyTitle: propertyTitles[Math.floor(Math.random() * propertyTitles.length)],
        propertyType: STUDIO_APARTMENT_TYPE.label,
        listingType,
        description: `A compact and modern studio apartment with efficient space utilization, available for ${listingType.toLowerCase()}.`,
