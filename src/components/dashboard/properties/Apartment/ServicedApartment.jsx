...
const ServicedApartment = () => {
  ...
  const normalizeProperties = useCallback((list) => {
    ...
    return list.map(p => ({
      ...p,
      propertyType: SERVICED_APARTMENT_TYPE.label
    }));
  }, []);
  ...
  const generateMockProperties = useCallback(() => {
    const propertyTitles = [
      'Silver Bay Serviced Apartment', 'Orchid Suites Serviced Stay', 'Palm Grove Serviced Residency',
      'Lake View Serviced Suites', 'Golden Tulip Serviced Apartment', 'Royal Palm Serviced Stay',
      'Emerald Serviced Apartments', 'Pearl Serviced Residency', 'Crystal Serviced Suites',
      'Sapphire Serviced Apartment', 'Ruby Serviced Stay', 'Diamond Serviced Residency',
      'Sunrise Serviced Apartments', 'Moonlight Serviced Suites', 'Star Serviced Residency',
      'Cloud Nine Serviced Stay', 'Harbor View Serviced Apartment', 'Garden Serviced Suites'
    ];
    ...
    for (let i = 1; i <= 40; i++) {
      ...
      propertiesList.push({
        id: `sa_${i}`,
        propertyId: `SA-${String(i).padStart(4, '0')}`,
        propertyTitle: propertyTitles[Math.floor(Math.random() * propertyTitles.length)],
        propertyType: SERVICED_APARTMENT_TYPE.label,
        listingType,
        description: `A well-maintained serviced apartment with premium amenities and prime connectivity, available for ${listingType.toLowerCase()}.`,
