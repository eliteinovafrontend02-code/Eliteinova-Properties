...
const LuxuryApartment = () => {
  ...
  const normalizeProperties = useCallback((list) => {
    ...
    return list.map(p => ({
      ...p,
      propertyType: LUXURY_APARTMENT_TYPE.label
    }));
  }, []);
  ...
  const generateMockProperties = useCallback(() => {
    const propertyTitles = [
      'Crown Heights Luxury Residence', 'Regal Luxury Suites', 'Royal Palm Luxury Apartments',
      'Golden Crest Luxury Residency', 'Platinum Luxury Tower', 'Diamond Luxury Suites',
      'Imperial Luxury Apartments', 'Majestic Luxury Residency', 'Grand Luxury Tower',
      'Elite Luxury Suites', 'Prestige Luxury Apartments', 'Opulent Luxury Residency',
      'Noble Luxury Tower', 'Regency Luxury Suites', 'Sovereign Luxury Apartments',
      'Victoria Luxury Residency', 'Windsor Luxury Tower', 'Buckingham Luxury Suites'
    ];
    ...
    for (let i = 1; i <= 40; i++) {
      ...
      propertiesList.push({
        id: `lx_${i}`,
        propertyId: `LX-${String(i).padStart(4, '0')}`,
        propertyTitle: propertyTitles[Math.floor(Math.random() * propertyTitles.length)],
        propertyType: LUXURY_APARTMENT_TYPE.label,
        listingType,
        description: `An exquisite luxury apartment with premium finishes and world-class amenities, available for ${listingType.toLowerCase()}.`,
