...
const EditPropertyModal = ({ property, show, onClose, onSave }) => {
  ...
  const handleSubmit = (e) => {
    ...
    setLoading(true);
    setTimeout(() => {
      onSave({ ...property, ...formData, propertyType: APARTMENT_TYPE.label, listingType: RENT_TYPE.label });
      setLoading(false);
      onClose();
    }, 700);
  };
...
const RentalApartment = () => {
  ...
  const generateMockProperties = useCallback(() => {
    const propertyTitles = [
      'Silver Bay Rental Apartment', 'Orchid Suites Rental Home', 'Palm Grove Rental Residency',
      'Lake View Rental Apartment', 'Golden Tulip Rental Flat', 'Royal Palm Rental Apartment',
      'Emerald Rental Apartments', 'Pearl Rental Residency', 'Crystal Rental Suites',
      'Sapphire Rental Apartment', 'Ruby Rental Flat', 'Diamond Rental Residency',
      'Sunrise Rental Apartments', 'Moonlight Rental Suites', 'Star Rental Residency',
      'Cloud Nine Rental Flat', 'Harbor View Rental Apartment', 'Garden Rental Suites'
    ];
    ...
    for (let i = 1; i <= 40; i++) {
      ...
      propertiesList.push({
        ...
        propertyType: APARTMENT_TYPE.label,
        listingType: RENT_TYPE.label,
        description: `A well-maintained rental apartment with modern amenities and prime connectivity, available for rent.`,
