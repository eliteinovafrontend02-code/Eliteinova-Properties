...
const IndependentHouse = () => {
  ...
  const generateMockProperties = useCallback(() => {
    const propertyTitles = [
      'Green Valley House', 'Lakeside Independent House', 'Sunrise Family House', 'Royal Palm House',
      'Silver Oak House', 'Golden Meadows House', 'Cedar Woods House', 'Maple Leaf House',
      'Orchid Garden House', 'Tulip Independent House', 'Lotus Family House', 'Jasmine House',
      'Emerald Greens House', 'Pearl Residency House', 'Ruby Enclave House', 'Sapphire House'
    ];
    ...
    for (let i = 1; i <= 40; i++) {
      ...
      propertiesList.push({
        id: `ih_${i}`,
        propertyId: `IH-${String(i).padStart(4, '0')}`,
        propertyTitle: propertyTitles[Math.floor(Math.random() * propertyTitles.length)],
        propertyType: HOUSE_TYPE.label,
        listingType,
        description: `A well-maintained independent house with good ventilation and prime connectivity, available for ${listingType.toLowerCase()}.`,
