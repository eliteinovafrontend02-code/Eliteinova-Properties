...
const IndividualOverview = () => {
  ...
  const computeStats = useCallback((list) => {
    ...
    const independentHouse = list.filter(p => p.propertyType === 'Independent House').length;
    const independentVilla = list.filter(p => p.propertyType === 'Independent Villa').length;
    const duplexResidentialUnit = list.filter(p => p.propertyType === 'Duplex Residential Unit').length;
    setStats({ total, independentHouse, independentVilla, duplexResidentialUnit });
  }, []);

  // ============ GENERATE MOCK DATA ============
  const generateMockProperties = useCallback(() => {
    const propertyTitles = [
      'Green Valley Home', 'Lakeside Independent House', 'Sunrise Villa', 'Royal Palm Villa',
      'Silver Oak House', 'Golden Meadows Duplex', 'Cedar Woods Villa', 'Maple Leaf House',
      'Orchid Garden Villa', 'Tulip Duplex Home', 'Lotus Independent House', 'Jasmine Villa',
      'Emerald Greens Duplex', 'Pearl Residency House', 'Ruby Enclave Villa', 'Sapphire Duplex'
    ];
    ...
    for (let i = 1; i <= 40; i++) {
      ...
      propertiesList.push({
        id: `ind_${i}`,
        propertyId: `IND-${String(i).padStart(4, '0')}`,
        ...
        description: `A well-maintained ${propertyType.toLowerCase()} with good ventilation and prime connectivity.`,
