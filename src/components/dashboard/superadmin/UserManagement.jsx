// src/components/dashboard/admin/superadmin/UserManagement.jsx
...
// Plan-specific colors (badge style like the Gold image)
const PLAN_COLORS = {
...
const USER_TYPE_CONFIG = {
  'Owner': { icon: FaHome, bg: 'bg-emerald-50', text: 'text-emerald-700', border: 'border-emerald-200', gradient: 'from-emerald-800 to-teal-600', color: '#10B981', description: 'Property owners who list and manage their properties' },
  'Agent': { icon: FiBriefcase, bg: 'bg-blue-50', text: 'text-blue-700', border: 'border-blue-200', gradient: 'from-blue-600 via-blue-500 to-indigo-400', color: '#3B82F6', description: 'Real estate agents who connect buyers and sellers' },
  'Builder': { icon: FaHardHat, bg: 'bg-orange-50', text: 'text-orange-700', border: 'border-orange-200', gradient: 'from-orange-600 via-orange-500 to-amber-400', color: '#F97316', description: 'Builders who construct and sell new properties' },
  'Property Manager': { icon: FiClipboard, bg: 'bg-purple-50', text: 'text-purple-700', border: 'border-purple-200', gradient: 'from-purple-600 via-purple-500 to-violet-400', color: '#8B5CF6', description: 'Managers who handle property management services' }
};
...
// ============================================================
// DATE RANGE PRESETS (matching PaymentReports)
// ============================================================
...
// ============================================================
// TIME SERIES LABEL GENERATOR (calendar-based, like PaymentReports)
// ============================================================
...
// Generate growth values with a smooth trending pattern per label
const generateGrowthSeries = (labels, seed = 1) => {
  const rand = seededRandom(seed);
  let base = Math.floor(rand() * 40) + 30;
  return labels.map((label) => {
    base += Math.floor(rand() * 20) - 5; // slight up/down drift
    return { label, value: Math.max(base, 15) };
  });
};

// ============================================================
// DATA GENERATORS (with date range support)
// ============================================================
const generateMockUsers = (type, count = 20, dateRange = 'month', customStart, customEnd) => {
  ...
  const plans = PLANS_BY_USER_TYPE[type] || ['Free'];
  const companies = ['ABC Realty', 'Dream Homes', 'Green Valley', 'Luxury Living', 'Urban Estate', 'Prime Properties', 'Elite Homes', 'Royal Estate'];
  const agencies = ['Skyline Realty', 'Trust Estates', 'Coastal Properties', 'Metro Housing', 'NextGen Realty', 'Golden Key Estates'];
  const builderCompanies = ['Shree Construction', 'BuildWell Developers', 'Green Valley Builders', 'Urban Homes Group', 'Premier Estates Ltd'];
  const pmCompanies = ['Prestige Property Management', 'Golden Gate Realty', 'Urban Nest Solutions', 'Elite Estate Managers'];
  ...
  return Array.from({ length: count }, (_, i) => {
    ...
    let companyName = companies[i % companies.length];
    if (type === 'Agent') companyName = agencies[i % agencies.length];
    if (type === 'Builder') companyName = builderCompanies[i % builderCompanies.length];
    if (type === 'Property Manager') companyName = pmCompanies[i % pmCompanies.length];
    ...
    return {
      ...
      propertiesCount: Math.floor(Math.random() * 30),
      leadsCount: Math.floor(Math.random() * 50),
      clientsCount: Math.floor(Math.random() * 40),
      projectsCount: Math.floor(Math.random() * 10),
      rating: (Math.random() * 4 + 1).toFixed(1),
      experience: Math.floor(Math.random() * 15) + 1,
      specialization: ALL_PROPERTY_TYPES[Math.floor(Math.random() * ALL_PROPERTY_TYPES.length)],
...
const generatePropertiesForUser = (user) => {
  ...
  return Array.from({ length: count }, (_, idx) => {
    ...
    return {
      ...
      ownerName: user.name,
      description: `${type} managed by ${user.name} in ${city}.`,
...
const DOC_TYPES_BY_USER_TYPE = {
  ...
  'Agent': [
    ...
    { key: 'aadhaar', label: 'Aadhaar Card' },
    { key: 'pan', label: 'PAN Card' },
    { key: 'agency', label: 'Agency License' }
  ],
  'Builder': [
...
const generateVerificationDocs = (user) => {
  const docTypes = DOC_TYPES_BY_USER_TYPE[user.type] || [];
...
// ============================================================
// BUILD USER TYPE STATS (accepts datePreset for calendar-based growth)
// ============================================================
const buildUserTypeStats = (type, userList, datePreset = 'month') => {
  const plans = PLANS_BY_USER_TYPE[type];

  // 👇 Dynamic calendar-based labels for monthly growth
  ...
  return {
    ...
    pendingKyc: userList.filter(u => u.kycStatus === 'pending').length,
    approvedAndVerified: userList.filter(u => u.status === 'approved' && u.kycStatus === 'verified').length,
    totalRevenue: userList.reduce((sum, u) => sum + u.revenue, 0),
    totalCommission: userList.reduce((sum, u) => sum + u.commission, 0),
    totalTransactions: userList.reduce((sum, u) => sum + u.transactionCount, 0),
    totalProperties: userList.reduce((sum, u) => sum + u.propertiesCount, 0),
    totalLeads: userList.reduce((sum, u) => sum + u.leadsCount, 0),
    avgRating: userList.length > 0
      ? (userList.reduce((sum, u) => sum + parseFloat(u.rating), 0) / userList.length).toFixed(1)
      : '0.0',
    planDistribution: plans.map((plan, idx) => ({
      label: plan,
      value: userList.filter(u => u.subscriptionPlan === plan).length,
      color: UNIQUE_COLORS[idx % UNIQUE_COLORS.length]
    })),
    statusDistribution: [
      { label: 'Approved', value: userList.filter(u => u.status === 'approved').length, color: '#10B981' },
      { label: 'Pending', value: userList.filter(u => u.status === 'pending').length, color: '#F59E0B' },
      { label: 'Rejected', value: userList.filter(u => u.status === 'rejected').length, color: '#EF4444' },
      { label: 'Blocked', value: userList.filter(u => u.status === 'blocked').length, color: '#6B7280' }
    ],
    revenueByPlan: plans.map((plan, idx) => ({
      label: plan,
      value: userList.filter(u => u.subscriptionPlan === plan).reduce((sum, u) => sum + u.revenue, 0),
      color: UNIQUE_COLORS[idx % UNIQUE_COLORS.length]
    })),
    // 👇 Now dynamic based on calendar
    monthlyGrowth
  };
};
...
// ============================================================
// CONFIRM MODAL (Custom Yes/No alert for Block/Unblock)
// ============================================================
...
const StatusBadge = ({ status }) => {
  ...
  const labels = {
    ...
    not_submitted: 'Not Submitted', suspended: 'Suspended', blocked: 'Blocked'
  };
  return (
...
// USER DETAILS MODAL
// ============================================================
const UserDetailsModal = ({ user, config, onClose, onToggleBlock }) => {
  const Icon = config.icon;
  const isBlocked = user.status === 'blocked' || user.status === 'rejected';
  const planStyle = getPlanStyle(user.subscriptionPlan);
  const PlanIcon = planStyle.icon;
  const docs = useMemo(() => generateVerificationDocs(user), [user]);
    ...
    <div className="fixed inset-0 z-[80] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
      <div className="bg-white rounded-3xl max-w-3xl w-full max-h-[90vh] overflow-hidden shadow-2xl animate-slide-up flex flex-col">
            ...
            <div className="min-w-0">
              ...
              <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                <span className="text-[10px] px-2 py-0.5 rounded-full bg-white/20 text-white font-bold flex items-center gap-1">
                  <Icon className="text-[10px]" /> {user.type}
                </span>
                <StatusBadge status={user.status} />
                {user.kycStatus === 'verified' && (
                  <span className="text-[10px] px-2 py-0.5 rounded-full bg-blue-500 text-white font-bold">KYC ✓</span>
                )}
              </div>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5 bg-[#F8FAF9]">
          ...
          <div className="bg-white rounded-2xl border border-[#E8F0EE] p-5">
            <h3 className="text-xs font-black text-[#3D5A55] uppercase tracking-wider mb-3 flex items-center gap-2">
              <FiPackage className="text-[#00695C]" /> Subscription Plan
            </h3>
            <div className="flex items-center justify-between gap-4 p-4 rounded-2xl border-2"
              style={{
                background: `linear-gradient(135deg, ${planStyle.bg} 0%, ${planStyle.bg}CC 100%)`,
                borderColor: planStyle.border
              }}>
              <div className="flex items-center gap-3">
                <div className="w-14 h-14 rounded-2xl flex items-center justify-center shadow-lg border-2"
                  style={{ backgroundColor: planStyle.bg, borderColor: planStyle.border }}>
                  <PlanIcon className="text-2xl" style={{ color: planStyle.text }} />
                </div>
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-wider" style={{ color: planStyle.text }}>Current Plan</p>
                  <PlanBadge plan={user.subscriptionPlan} size="lg" />
                </div>
              </div>
              <div className="text-right">
                <span className="inline-flex items-center gap-1 px-3 py-1.5 rounded-full text-xs font-bold border-2"
                  style={{ backgroundColor: planStyle.bg, color: planStyle.text, borderColor: planStyle.border }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-white rounded-2xl border border-[#E8F0EE] p-5">
              ...
              <div className="space-y-3">
                ...
                <InfoRow icon={<FiMapPin />} label="Location" value={`${user.city}, ${user.state}`} />
                <InfoRow icon={<FiBriefcase />} label="Company" value={user.company} />
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-[#E8F0EE] p-5">
              ...
              <div className="space-y-3">
                <InfoRow icon={<FiCalendar />} label="Registered" value={formatDate(user.registrationDate)} />
                <InfoRow icon={<FiShield />} label="KYC Status"
                  value={<StatusBadge status={user.kycStatus === 'verified' ? 'approved' : user.kycStatus} />} />
                <InfoRow icon={<FiBriefcase />} label="Experience" value={`${user.experience} years`} />
                <InfoRow icon={<FiLayers />} label="Specialization" value={user.specialization} />
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-[#E8F0EE] p-5">
            <h3 className="text-xs font-black text-[#3D5A55] uppercase tracking-wider mb-3 flex items-center gap-2">
              <FiActivity className="text-[#00695C]" /> Performance Stats
            </h3>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <MiniStat label="Properties" value={user.propertiesCount} color={config.color} />
              <MiniStat label="Leads" value={user.leadsCount} color="#3B82F6" />
              <MiniStat label="Revenue" value={formatCompact(user.revenue)} color="#10B981" />
              <MiniStat label="Rating" value={`★ ${user.rating}`} color="#F59E0B" />
              <MiniStat label="Transactions" value={user.transactionCount} color="#8B5CF6" />
              <MiniStat label="Commission" value={formatCompact(user.commission)} color="#EC4899" />
              <MiniStat label="Success Rate" value={`${user.successRate}%`} color="#06B6D4" />
              <MiniStat label="Clients" value={user.clientsCount} color="#F97316" />
            </div>
          </div>
        ...
        <div className="sticky bottom-0 px-6 py-4 bg-white border-t border-[#E8F0EE] flex items-center gap-3">
          <button onClick={() => { onToggleBlock(user); onClose(); }}
...
const OverviewTab = ({ activeUserType, stats, config, dateRangeLabel, datePreset }) => {
  ...
  // 👇 Dynamic growth chart title based on selected calendar
  const growthTitle = {
    today: 'Hourly Growth',
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        {/* 👇 DYNAMIC GROWTH CHART — labels change with calendar */}
        <div className="bg-white rounded-2xl p-6 border border-[#E8F0EE] shadow-sm">
...
const RegistrationsTab = ({
  ...
  useEffect(() => { setViewingUser(null); }, [userType]);
    ...
    <div className="space-y-4">
      <div className="bg-white rounded-2xl p-4 border border-[#E8F0EE] shadow-sm">
        <div className="flex flex-col lg:flex-row items-start lg:items-center gap-4">
          <div className="flex-1 w-full relative">
            ...
            <input type="text" placeholder={`Search ${userType.toLowerCase()}s...`} value={searchQuery}
      ...
      {viewMode === 'grid' ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-4">
          {filtered.map((user, index) => (
            <UserRegistrationCard key={user.id} user={user} config={config} index={index}
              onView={setViewingUser} onToggleBlock={onToggleBlock}
        ...
        <div className="space-y-3">
          {filtered.map((user, index) => (
            <UserRegistrationListCard key={user.id} user={user} config={config} index={index}
              onView={setViewingUser} onToggleBlock={onToggleBlock}
              actionLoading={actionLoading} />
          ))}
        </div>
      ...
      {filtered.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 text-center bg-white rounded-2xl border border-[#E8F0EE]">
          ...
          <h3 className="text-xl font-black text-[#0F1A18]">No {userType.toLowerCase()}s found</h3>
...
// Grid Card
const UserRegistrationCard = ({ user, config, index, onView, onToggleBlock, actionLoading }) => {
  const isBlocked = user.status === 'blocked' || user.status === 'rejected';
    ...
    <div className={`bg-white rounded-2xl border border-[#E8F0EE] p-4 hover:shadow-xl hover:-translate-y-1 transition-all duration-500 animate-card-in ${STATUS_BORDER[user.status] ? `border-l-4 ${STATUS_BORDER[user.status]}` : ''}`}
      ...
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <h4 className="font-bold text-[#0F1A18] text-sm truncate">{user.name}</h4>
      ...
      <div className="mb-3">
        <PlanBadge plan={user.subscriptionPlan} size="sm" />
      </div>

      <div className="space-y-1 text-[11px] text-[#3D5A55]">
        <div className="flex items-center gap-2"><FiMail /> <span className="truncate">{user.email}</span></div>
        <div className="flex items-center gap-2"><FiPhone /> <span>{user.phone}</span></div>
        <div className="flex items-center gap-2"><FiMapPin /> <span className="truncate">{user.city}, {user.state}</span></div>
        <div className="flex items-center gap-2"><FiCalendar /> <span>{formatDate(user.registrationDate)}</span></div>
        <div className="flex items-center gap-2"><FiBriefcase /> <span className="truncate">{user.company}</span></div>
        <div className="flex items-center gap-2"><FiTrendingUp /> <span>Revenue: {formatCompact(user.revenue)}</span></div>
      </div>

      <div className="grid grid-cols-3 gap-2 mt-3 pt-3 border-t border-[#E8F0EE]">
        <div className="text-center">
          <p className="text-sm font-black text-[#0F1A18]">{user.propertiesCount}</p>
          <p className="text-[8px] text-[#3D5A55] uppercase tracking-wider font-bold">Props</p>
        </div>
        <div className="text-center">
          <p className="text-sm font-black text-[#0F1A18]">{user.leadsCount}</p>
        ...
        <div className="text-center">
          <p className="text-sm font-black flex items-center justify-center gap-0.5" style={{ color: config.color }}>
            <FaStarSolid className="text-amber-400 text-[10px]" />{user.rating}
          </p>
...
// List View Card 
const UserRegistrationListCard = ({ user, config, index, onView, onToggleBlock, actionLoading }) => {
  const isBlocked = user.status === 'blocked' || user.status === 'rejected';
  ...
  return (
    ...
    >
      {/* ===== ROW 1: HEADER (Avatar + Name + Badges + Actions) ===== */}
        ...
        <div className="flex items-center gap-3 min-w-0 flex-1">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 flex-wrap">
              <h4 className="font-bold text-[15px] text-[#0F1A18] truncate">
                {user.name}
              </h4>
              <StatusBadge status={user.status} />
              <PlanBadge plan={user.subscriptionPlan} size="sm" />
      ...
      {/* ===== ROW 2: DETAILS GRID (Contact, Location, Registered, Revenue) ===== */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 px-5 py-4">
        ...
        <div className="min-w-0">
          ...
          <p className="text-[13px] font-semibold text-[#1A2E2A] truncate">{user.email}</p>
          <p className="text-[12px] text-[#3D5A55] font-medium mt-0.5">{user.phone}</p>
        </div>
        ...
        <div className="min-w-0">
          ...
          <p className="text-[13px] font-semibold text-[#1A2E2A] truncate">{user.city}</p>
          <p className="text-[12px] text-[#3D5A55] font-medium mt-0.5 truncate">{user.state}</p>
        </div>
        ...
        <div className="min-w-0">
          ...
          <p className="text-[13px] font-semibold text-[#1A2E2A]">{formatDate(user.registrationDate)}</p>
        ...
        <div className="min-w-0">
          ...
          <p className="text-[15px] font-black text-[#00695C]">{formatCompact(user.revenue)}</p>
          <p className="text-[12px] text-[#3D5A55] font-medium mt-0.5">
      ...
      {/* ===== ROW 3: STATS STRIP (Properties, Leads, Rating) ===== */}
      <div className="grid grid-cols-3 gap-3 px-5 pb-4">
        <div className="flex items-center gap-2.5 px-4 py-2.5 bg-[#F5F9F8] rounded-xl border border-[#E8F0EE]">
          ...
          <div className="min-w-0">
            <p className="text-[15px] font-black text-[#0F1A18] leading-none">{user.propertiesCount}</p>
            <p className="text-[10px] text-[#3D5A55] uppercase tracking-wider font-bold mt-1">Properties</p>
          </div>
        </div>

        <div className="flex items-center gap-2.5 px-4 py-2.5 bg-[#F5F9F8] rounded-xl border border-[#E8F0EE]">
          <FiUserPlus className="text-[#3B82F6] text-base flex-shrink-0" />
          <div className="min-w-0">
            <p className="text-[15px] font-black text-[#0F1A18] leading-none">{user.leadsCount}</p>
            <p className="text-[10px] text-[#3D5A55] uppercase tracking-wider font-bold mt-1">Leads</p>
          </div>
        </div>

        <div className="flex items-center gap-2.5 px-4 py-2.5 bg-[#F5F9F8] rounded-xl border border-[#E8F0EE]">
          <FaStarSolid className="text-amber-400 text-sm flex-shrink-0" />
          <div className="min-w-0">
            <p className="text-[15px] font-black text-[#0F1A18] leading-none">{user.rating}</p>
            <p className="text-[10px] text-[#3D5A55] uppercase tracking-wider font-bold mt-1">Rating</p>
          </div>
        </div>
      </div>
    </div>
              ...
              <div key={user.id} className={`bg-white rounded-2xl border border-[#E8F0EE] p-4 hover:shadow-xl transition animate-card-in ${STATUS_BORDER[status] ? `border-l-4 ${STATUS_BORDER[status]}` : ''}`}
                ...
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <h4 className="font-bold text-sm text-[#0F1A18] truncate">{user.name}</h4>
                      <p className="text-[10px] text-[#3D5A55] truncate">{user.email}</p>
                    </div>
                  </div>
            ...
            return (
              <div key={user.id} className="grid grid-cols-12 gap-2 items-center py-3 px-4 border-b border-[#E8F0EE] hover:bg-[#F5F9F8] transition">
                <div className="col-span-4 flex items-center gap-2 min-w-0">
                  ...
                  <div className="min-w-0">
                    <p className="font-bold text-sm text-[#1A2E2A] truncate">{user.name}</p>
                    <p className="text-[10px] text-[#3D5A55] truncate">{user.email}</p>
                  </div>
                </div>
          ...
          <div className="bg-white rounded-3xl max-w-2xl w-full max-h-[85vh] overflow-hidden shadow-2xl animate-slide-up flex flex-col">
            <div className="sticky top-0 bg-gradient-to-r from-[#00695C] to-[#26A69A] px-5 py-3 rounded-t-3xl z-10 shrink-0 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h2 className="text-base font-bold text-white">{selectedUser.name}'s Verification</h2>
...
const PropertiesTab = ({ userType, users, showToast, dateRangeLabel }) => {
  ...
  useEffect(() => {
    if (viewingUser) {
      setProperties(generatePropertiesForUser(viewingUser));
    }
  }, [viewingUser]);
        ...
        {users.map((user, index) => (
          <div key={user.id} className="bg-white rounded-2xl border border-[#E8F0EE] p-4 hover:shadow-xl hover:-translate-y-1 transition-all duration-500 animate-card-in"
            ...
            <div className="flex items-center gap-3 mb-3">
              ...
              <div className="min-w-0">
                <h4 className="font-bold text-sm text-[#0F1A18] truncate">{user.name}</h4>
                <p className="text-[10px] text-[#3D5A55] truncate">{user.company}</p>
              </div>
            </div>
            <div className="space-y-1 text-[11px] text-[#3D5A55]">
              <div className="flex items-center gap-2"><FiMail /> <span className="truncate">{user.email}</span></div>
            ...
            <div className="grid grid-cols-3 gap-2 mt-3 pt-3 border-t border-[#E8F0EE]">
              ...
              <div className="text-center">
                <p className="text-sm font-black" style={{ color: config.color }}>{user.rating}</p>
                <p className="text-[8px] text-[#3D5A55] uppercase tracking-wider font-bold">Rating</p>
              </div>
            </div>
            <button onClick={() => setViewingUserId(user.id)}
              ...
              <FiEye className="text-[10px]" /> Manage Properties ({user.propertiesCount})
            </button>
          </div>
        ))}
      </div>
          ...
          <div className="bg-white rounded-3xl max-w-4xl w-full max-h-[85vh] overflow-hidden shadow-2xl animate-slide-up flex flex-col">
            <div className="sticky top-0 bg-gradient-to-r from-[#00695C] to-[#26A69A] px-5 py-3 rounded-t-3xl z-10 shrink-0 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h2 className="text-base font-bold text-white">{viewingUser.name}'s Properties</h2>
        ...
        {users.map((user, index) => (
          <div key={user.id} className="bg-white rounded-2xl border border-[#E8F0EE] p-4 hover:shadow-xl hover:-translate-y-1 transition-all duration-500 animate-card-in"
            ...
            <div className="flex items-center gap-3 mb-3">
              ...
              <div className="min-w-0">
                <h4 className="font-bold text-sm text-[#0F1A18] truncate">{user.name}</h4>
                <p className="text-[10px] text-[#3D5A55] truncate">{user.city}</p>
              </div>
            </div>
            <div className="grid grid-cols-3 gap-2 mb-3">
              <div className="text-center p-2 bg-[#F5F9F8] rounded-lg">
                <p className="text-sm font-black text-[#00695C]">{formatCompact(user.commission)}</p>
                <p className="text-[8px] text-[#3D5A55] uppercase tracking-wider font-bold">Commission</p>
              </div>
              <div className="text-center p-2 bg-[#F5F9F8] rounded-lg">
                <p className="text-sm font-black text-[#0F1A18]">{user.leadsCount}</p>
                <p className="text-[8px] text-[#3D5A55] uppercase tracking-wider font-bold">Leads</p>
              ...
              <div className="text-center p-2 bg-[#F5F9F8] rounded-lg">
                <p className="text-sm font-black text-[#0F1A18]">{user.rating}</p>
                <p className="text-[8px] text-[#3D5A55] uppercase tracking-wider font-bold">Rating</p>
              </div>
            </div>
            <button onClick={() => setViewingUserId(user.id)}
        ...
        <div className="sticky top-0 bg-gradient-to-r from-[#00695C] to-[#26A69A] px-5 py-3 rounded-t-3xl z-10 shrink-0 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-white/90 text-[10px]">{user.name}</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div className="text-center p-3 bg-[#F5F9F8] rounded-xl border-l-4 border-l-[#00695C]">
              ...
              <p className="text-[9px] uppercase tracking-wider text-[#3D5A55] font-bold">Properties</p>
            </div>
            <div className="text-center p-3 bg-[#F5F9F8] rounded-xl border-l-4 border-l-blue-500">
              ...
              <p className="text-[9px] uppercase tracking-wider text-[#3D5A55] font-bold">Leads</p>
            </div>
                ...
                {performanceData.monthlyPerformance.map((item) => (
                  <div key={item.month} className="flex-1 flex flex-col items-center gap-1">
                    <div className="w-full flex flex-col items-center gap-0.5">
                      <div className="w-full rounded-t-lg bg-gradient-to-t from-[#00695C] to-[#26A69A]" style={{ height: `${(item.properties / maxValue) * 80}px`, minHeight: '8px' }} />
                      <span className="text-[9px] font-bold text-[#1A2E2A]">{item.properties}</span>
                    </div>
...
const UserManagement = () => {
  const [activeUserType, setActiveUserType] = useState('Owner');
  ...
  // 👇 Confirmation modal state (Block / Unblock)
  ...
  // Regenerate users when date range changes
  useEffect(() => {
    const newUsers = {};
    const newStats = {};
    ALL_USER_TYPES.forEach(type => {
      const userList = generateMockUsers(type, Math.floor(Math.random() * 20) + 15, datePreset, customStart, customEnd);
      newUsers[type] = userList;
      newStats[type] = buildUserTypeStats(type, userList, datePreset);
    });
    setUsers(newUsers);
  ...
  // ============================================================
  // BLOCK / UNBLOCK — Step 1: request confirmation (opens modal)
  // ============================================================
  const requestToggleBlock = useCallback((user) => {
    const isBlocked = user.status === 'blocked' || user.status === 'rejected';
    setConfirmAction({
      user,
      isBlocked,
      ...
      message: isBlocked
        ...
        : `Are you sure you want to block ${user.name}? They will lose access to the platform immediately.`,
  ...
  // ============================================================
  // BLOCK / UNBLOCK — Step 2: execute after "Yes" clicked
  // ============================================================
  const executeToggleBlock = useCallback(() => {
    if (!confirmAction) return;
    const { user, isBlocked } = confirmAction;

    setUsers(prev => {
      const updatedList = prev[activeUserType].map(u =>
        u.id === user.id ? { ...u, status: isBlocked ? 'approved' : 'blocked' } : u
      );
      const updated = { ...prev, [activeUserType]: updatedList };
      setUserTypeStats(prevStats => ({
        ...prevStats,
        [activeUserType]: buildUserTypeStats(activeUserType, updatedList, datePreset)
      }));
      return updated;
    });

    showToast(`${user.name} ${isBlocked ? 'unblocked' : 'blocked'}`, isBlocked ? 'success' : 'warning');
  ...
  const handleRefresh = useCallback(() => {
    ...
    setTimeout(() => {
      const newUsers = generateMockUsers(activeUserType, Math.floor(Math.random() * 20) + 15, datePreset, customStart, customEnd);
      setUsers(prev => ({ ...prev, [activeUserType]: newUsers }));
      setUserTypeStats(prev => ({ ...prev, [activeUserType]: buildUserTypeStats(activeUserType, newUsers, datePreset) }));
      setActionLoading(null);
  ...
  return (
    <div className="space-y-6 p-4 lg:p-6 min-h-screen bg-[#F8FAF9]">
      ...
      {/* 👇 Custom Block/Unblock confirmation modal */}
      <ConfirmModal
