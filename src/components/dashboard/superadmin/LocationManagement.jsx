// src/components/dashboard/admin/superadmin/LocationManagement.jsx
...
import {
  ...
  FiCompass, FiNavigation, FiCrosshair
} from 'react-icons/fi';
import {
...
import {
  ...
  MdOutlineApartment, MdLocationCity
} from 'react-icons/md';

// ============================================================
// LOCATION CATEGORY CONFIG  (same colors as Property/Project)
// ============================================================
const LOCATION_CATEGORIES = {
  'Individual': {
    ...
    label: 'Individual',
    description: 'Individual house, villa and duplex locations across cities',
  ...
  'Apartment': {
    ...
    label: 'Apartment',
    description: 'Apartment communities, gated societies and towers',
  ...
  'Commercial': {
    ...
    label: 'Commercial',
    description: 'Commercial hubs, business parks, retail and IT corridors',
  ...
  'Land & Plots': {
    ...
    label: 'Land & Plots',
    description: 'Land parcels, plots, agricultural and industrial zones',
  ...
  'Hostel': {
    ...
    label: 'Hostel',
    description: 'Hostel, PG and co-living zones across cities and suburbs',
    gradient: 'from-rose-600 via-rose-500 to-pink-400',
    hex: '#EC4899'
  }
};
const ALL_CATEGORIES = Object.keys(LOCATION_CATEGORIES);
...
const STATUS_CONFIG = {
  ...
  'Popular': { color: 'bg-blue-500', text: 'text-blue-700', bg: 'bg-blue-50', border: 'border-blue-200', label: 'Popular' },
  'Trending': { color: 'bg-purple-600', text: 'text-purple-700', bg: 'bg-purple-50', border: 'border-purple-200', label: 'Trending' },
  'Emerging': { color: 'bg-amber-500', text: 'text-amber-700', bg: 'bg-amber-50', border: 'border-amber-200', label: 'Emerging' },
  'Inactive': { color: 'bg-gray-500', text: 'text-gray-700', bg: 'bg-gray-50', border: 'border-gray-200', label: 'Inactive' }
};
...
// Sub-locations per category (kept for display as a tag)
const SUB_LOCATIONS = {
  'Individual': ['Independent House Zone', 'Villa Community', 'Duplex Enclave', 'Row House Colony', 'Premium Residential Zone'],
  'Apartment': ['Gated Apartment Zone', 'High-Rise Tower Cluster', 'Serviced Apartment Belt', 'Luxury Condo Corridor', 'Studio & 1BHK Cluster'],
  'Commercial': ['IT Park Corridor', 'Business District', 'Retail Market Zone', 'Showroom Belt', 'Industrial Park', 'Co-Working Hub'],
  'Land & Plots': ['Residential Plot Zone', 'Commercial Plot Zone', 'Agricultural Belt', 'Industrial Land Zone', 'Investment Land Corridor'],
  'Hostel': ['Girls Hostel Zone', 'Boys Hostel Zone', 'Co-Living Hub', 'Working Professional Cluster', 'Student Housing Belt']
};

// City-state-district mapping (correct pairing)
const CITY_MAPPING = [
  { city: 'Kochi', state: 'Kerala', district: 'Ernakulam', areas: ['Kakkanad', 'Edappally', 'Fort Kochi', 'Anna Nagar'] },
  { city: 'Mumbai', state: 'Maharashtra', district: 'Mumbai Suburban', areas: ['Bandra', 'Andheri', 'Powai', 'Worli'] },
  { city: 'Bangalore', state: 'Karnataka', district: 'Bengaluru Urban', areas: ['Koramangala', 'Indiranagar', 'Whitefield', 'Electronic City', 'Marathahalli'] },
  { city: 'Chennai', state: 'Tamil Nadu', district: 'Chennai', areas: ['Anna Nagar', 'Adyar', 'T Nagar'] },
  { city: 'Hyderabad', state: 'Telangana', district: 'Hyderabad', areas: ['Jubilee Hills', 'Gachibowli', 'Hitech City', 'Banjara Hills'] },
  { city: 'Pune', state: 'Maharashtra', district: 'Pune', areas: ['Koregaon Park', 'Kharadi', 'Viman Nagar'] },
  { city: 'Delhi', state: 'Delhi', district: 'New Delhi', areas: ['Connaught Place', 'Saket', 'Dwarka'] },
  { city: 'Ahmedabad', state: 'Gujarat', district: 'Ahmedabad', areas: ['Vastrapur', 'SG Highway', 'Satellite'] },
  { city: 'Kolkata', state: 'West Bengal', district: 'Kolkata', areas: ['Salt Lake', 'New Town', 'Park Street'] },
  { city: 'Jaipur', state: 'Rajasthan', district: 'Jaipur', areas: ['Malviya Nagar', 'Vaishali Nagar', 'Mansarovar'] }
];

const LANDMARKS = ['Airport Road', 'Metro Station', 'City Mall', 'Tech Park', 'Central Park', 'Railway Station', 'Bus Terminal', 'Ring Road', 'Beach Road', 'Lake View'];
...
// ============================================================
// DATA GENERATOR — locations  ✅ FIXED: locationName = area only
// ============================================================
const generateMockLocations = (category, count = 25, dateRange = 'month', customStart, customEnd) => {
  const subLocs = SUB_LOCATIONS[category] || ['General'];
  const statuses = ['Active', 'Popular', 'Trending', 'Emerging', 'Inactive'];
  ...
  return Array.from({ length: count }, (_, i) => {
    ...
    const cityData = CITY_MAPPING[Math.floor(Math.random() * CITY_MAPPING.length)];
    const area = cityData.areas[Math.floor(Math.random() * cityData.areas.length)];
    const subLoc = subLocs[Math.floor(Math.random() * subLocs.length)];
    const landmark = LANDMARKS[Math.floor(Math.random() * LANDMARKS.length)];

    return {
      id: `loc_${category.toLowerCase().replace(/[^a-z0-9]+/g, '_')}_${i + 1}_${Date.now()}`,
      ...
      latitude: (11 + Math.random() * 15).toFixed(4),
      longitude: (72 + Math.random() * 15).toFixed(4),
      ...
      activeProperties: 0,
      avgPrice: Math.floor(Math.random() * 45000000) + 5000000,
      totalValue: Math.floor(Math.random() * 500000000) + 50000000,
      views: Math.floor(Math.random() * 5000) + 200,
      inquiries: Math.floor(Math.random() * 400) + 10,
      areaSqkm: (Math.random() * 20 + 1).toFixed(2),
      demandScore: Math.floor(Math.random() * 100) + 1,
      description: `Prime ${category.toLowerCase()} location in ${cityData.city}, ${cityData.state} with excellent connectivity and amenities.`,
      postedDate: postedDate.toISOString().split('T')[0]
    };
  }).map(l => {
    l.activeProperties = Math.floor(l.totalProperties * (Math.random() * 0.7 + 0.2));
    return l;
  });
};
...
// BUILD STATS
// ============================================================
const buildLocationStats = (category, list, datePreset = 'month') => {
  ...
  const totalValue = list.reduce((sum, l) => sum + (l.totalValue || 0), 0);

  return {
    total: list.length,
    active: list.filter(l => l.status === 'Active').length,
    popular: list.filter(l => l.status === 'Popular').length,
    trending: list.filter(l => l.status === 'Trending').length,
    emerging: list.filter(l => l.status === 'Emerging').length,
    inactive: list.filter(l => l.status === 'Inactive').length,
    featured: list.filter(l => l.featured).length,
    verified: list.filter(l => l.verificationStatus === 'Verified').length,
    notVerified: list.filter(l => l.verificationStatus === 'Not Verified').length,
    totalValue,
    totalProperties: list.reduce((sum, l) => sum + (l.totalProperties || 0), 0),
    activeProperties: list.reduce((sum, l) => sum + (l.activeProperties || 0), 0),
    totalViews: list.reduce((sum, l) => sum + (l.views || 0), 0),
    totalInquiries: list.reduce((sum, l) => sum + (l.inquiries || 0), 0),
    avgPrice: list.length > 0 ? Math.floor(totalValue / list.length) : 0,
    avgDemandScore: list.length > 0
      ? Math.round(list.reduce((sum, l) => sum + (l.demandScore || 0), 0) / list.length)
      : 0,
    statusDistribution: [
      { label: 'Active', value: list.filter(l => l.status === 'Active').length, color: '#10B981' },
      { label: 'Popular', value: list.filter(l => l.status === 'Popular').length, color: '#3B82F6' },
      { label: 'Trending', value: list.filter(l => l.status === 'Trending').length, color: '#8B5CF6' },
      { label: 'Emerging', value: list.filter(l => l.status === 'Emerging').length, color: '#F59E0B' },
      { label: 'Inactive', value: list.filter(l => l.status === 'Inactive').length, color: '#6B7280' }
    ].filter(d => d.value > 0),
    revenueByStatus: [
      { label: 'Active', value: list.filter(l => l.status === 'Active').reduce((s, l) => s + (l.totalValue || 0), 0), color: '#10B981' },
      { label: 'Popular', value: list.filter(l => l.status === 'Popular').reduce((s, l) => s + (l.totalValue || 0), 0), color: '#3B82F6' },
      { label: 'Trending', value: list.filter(l => l.status === 'Trending').reduce((s, l) => s + (l.totalValue || 0), 0), color: '#8B5CF6' },
      { label: 'Emerging', value: list.filter(l => l.status === 'Emerging').reduce((s, l) => s + (l.totalValue || 0), 0), color: '#F59E0B' }
    ].filter(d => d.value > 0),
    monthlyGrowth
  };
};
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard icon={<FiMapPin className="text-white text-base" />} title={`Total ${activeCategory} Locations`} value={stats.total || 0} trend={12.5} subtitle="All locations" color={`bg-gradient-to-br ${config.gradient}`} delay={0} />
        <StatCard icon={<FiActivity className="text-white text-base" />} title="Active Locations" value={stats.active || 0} trend={8.3} subtitle="Live locations" color="bg-gradient-to-br from-blue-600 to-cyan-400" delay={100} />
        <StatCard icon={<FiTrendingUp className="text-white text-base" />} title="Trending" value={stats.trending || 0} trend={15.2} subtitle="High-demand zones" color="bg-gradient-to-br from-purple-600 to-violet-400" delay={200} />
        <StatCard icon={<FiDollarSign className="text-white text-base" />} title="Total Value" value={formatCompact(stats.totalValue || 0)} trend={10.5} subtitle="Combined property value" color="bg-gradient-to-br from-amber-600 to-orange-400" delay={300} />
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl p-6 border border-[#E8F0EE] shadow-sm">
          <h3 className="text-lg font-black text-[#0F1A18] mb-4 flex items-center gap-2">
            <FiDollarSign className="text-[#00695C]" /> Value by Status
          </h3>
...
// ============================================================
const ViewLocationDetailModal = ({ location, show, onClose, onDelete }) => {
  if (!location || !show) return null;

  const categoryConfig = LOCATION_CATEGORIES[location.category] || LOCATION_CATEGORIES['Individual'];
  const CategoryIcon = categoryConfig.icon;
  const statusConfig = STATUS_CONFIG[location.status] || STATUS_CONFIG['Active'];
      ...
      <div className="bg-white rounded-3xl max-w-4xl w-full max-h-[90vh] overflow-hidden shadow-2xl animate-slide-up border border-[#E8F0EE] flex flex-col">
        <div className="sticky top-0 bg-gradient-to-r from-[#00695C] to-[#26A69A] p-6 rounded-t-3xl z-10 shrink-0">
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <h2 className="text-2xl font-bold text-white truncate">{location.locationName}</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-white">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[
              { icon: <FiHash />, label: 'Location ID', value: location.locationId },
              { icon: <FiMapPin />, label: 'Location Name', value: location.locationName },
              { icon: <FiTag />, label: 'Category', value: location.category },
              { icon: <FiBriefcase />, label: 'Sub-Location', value: location.subLocation },
              { icon: <FiCompass />, label: 'Landmark', value: location.landmark },
              { icon: <FaCity />, label: 'District', value: location.district },
              { icon: <FaCity />, label: 'City', value: location.city },
              { icon: <FiMap />, label: 'State', value: location.state },
              { icon: <FiHash />, label: 'Pincode', value: location.pincode },
              { icon: <FiCompass />, label: 'Latitude', value: location.latitude },
              { icon: <FiCompass />, label: 'Longitude', value: location.longitude },
              { icon: <FiSquare />, label: 'Area', value: `${location.areaSqkm} sq km` },
              { icon: <FiHome />, label: 'Total Properties', value: location.totalProperties },
              { icon: <FiCheckCircle />, label: 'Active Properties', value: location.activeProperties },
              { icon: <FiDollarSign />, label: 'Avg Price', value: formatCompact(location.avgPrice) },
              { icon: <FiTrendingUp />, label: 'Demand Score', value: `${location.demandScore}/100` }
            ].map((item, i) => (
            ...
            <div className="bg-[#F5F9F8] rounded-2xl p-4 md:col-span-2">
              ...
              <p className="text-sm font-bold text-[#1A2E2A]">{location.description || 'No description available'}</p>
            </div>
          </div>
        </div>
        ...
        <div className="sticky bottom-0 px-6 py-4 bg-white border-t border-[#E8F0EE] rounded-b-3xl shrink-0 shadow-[0_-4px_20px_rgba(0,0,0,0.05)]">
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => { if (onDelete) { onDelete(location.id); } }} className="flex-1 px-4 py-2.5 bg-red-600 text-white rounded-xl hover:bg-red-700 text-sm font-medium shadow-lg">
...
// ============================================================
// CATEGORY MANAGE TAB  (sub-location filter removed)
// ============================================================
const CategoryManageTab = ({
  activeCategory, locations, onView, onDelete,
  ...
  const filtered = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      result = result.filter(l =>
        ...
        (l.locationId && l.locationId.toLowerCase().includes(q)) ||
        (l.subLocation && l.subLocation.toLowerCase().includes(q)) ||
    ...
    if (statusFilter !== 'All') {
      ...
      if (matchKey) result = result.filter(l => l.status === matchKey);
    }
    return result;
  ...
  const config = LOCATION_CATEGORIES[activeCategory];
      ...
      <div ref={filterBarRef} className="relative z-[100] bg-white rounded-2xl p-4 border border-[#E8F0EE] shadow-sm">
        <div className="flex flex-col lg:flex-row items-stretch lg:items-center gap-3">
          <div className="flex-1 relative min-w-0">
            ...
            <input
              type="text"
              placeholder={`Search by location ID, name, sub-location, city, landmark...`}
              value={searchQuery}
        ...
        <div className="mt-3 flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-2 text-[10px] text-[#3D5A55] font-semibold">
            <FiCalendar className="text-[#00695C]" /> Showing locations from: {dateRangeLabel}
          </div>
      ...
      {filtered.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 text-center bg-white rounded-2xl border border-[#E8F0EE]">
          <div className="w-24 h-24 rounded-full bg-[#F5F9F8] flex items-center justify-center mb-4 animate-float">
            <FiMapPin className="text-4xl text-[#8FA8A4]" />
          </div>
          <h3 className="text-xl font-black text-[#0F1A18]">No locations found</h3>
...
// ============================================================
// GRID CARD — Properties | Active | Views
// ============================================================
const LocationGridCard = ({ location, config, index, onView, onDelete, actionLoading }) => {
    ...
    >
      <div className="flex items-start gap-3 mb-3">
        ...
        <div className="min-w-0 flex-1">
          <h4 className="font-bold text-[#0F1A18] text-sm leading-tight line-clamp-2" title={location.locationName}>
            {location.locationName}
          </h4>
          <p className="text-[10px] text-[#5A7D78] font-bold mt-0.5 truncate">{location.locationId}</p>
        </div>
      </div>
      ...
      <div className="space-y-1.5 text-[11px] text-[#3D5A55] flex-1">
        <div className="flex items-center gap-2"><FiTag className="flex-shrink-0" /> <span className="truncate">{location.subLocation}</span></div>
        <div className="flex items-center gap-2"><FiMapPin className="flex-shrink-0" /> <span className="truncate">{location.city}, {location.state}</span></div>
        <div className="flex items-center gap-2"><FiCompass className="flex-shrink-0" /> <span className="truncate">{location.landmark}</span></div>
        <div className="flex items-center gap-2"><FiDollarSign className="flex-shrink-0" /> <span className="font-semibold text-[#1A2E2A]">{formatCompact(location.avgPrice)} avg</span></div>
        <div className="flex items-center gap-2"><FiTrendingUp className="flex-shrink-0" /> <span>Demand: {location.demandScore}/100</span></div>
      </div>

      <div className="grid grid-cols-3 gap-2 mt-3 pt-3 border-t border-[#E8F0EE]">
        <div className="text-center">
          <p className="text-sm font-black text-[#0F1A18]">{location.totalProperties}</p>
          <p className="text-[8px] text-[#3D5A55] uppercase tracking-wider font-bold">Properties</p>
        </div>
        <div className="text-center">
          <p className="text-sm font-black text-[#00695C]">{location.activeProperties}</p>
          <p className="text-[8px] text-[#3D5A55] uppercase tracking-wider font-bold">Active</p>
        </div>
        <div className="text-center">
          <p className="text-sm font-black" style={{ color: config.hex }}>{location.views}</p>
...
// ============================================================
// LIST CARD — Properties | Active | Views
// ============================================================
const LocationListCard = ({ location, config, index, onView, onDelete, actionLoading }) => {
  const CategoryIcon = config.icon;
    ...
    >
      <div className="flex items-center justify-between gap-4 px-5 py-4 border-b border-[#E8F0EE]">
        <div className="flex items-center gap-3 min-w-0 flex-1">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 flex-wrap">
              <h4 className="font-bold text-[15px] text-[#0F1A18] truncate">{location.locationName}</h4>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 px-5 py-4">
        <div className="min-w-0">
          ...
          <p className="text-[13px] font-semibold text-[#1A2E2A] truncate">{location.city}</p>
          <p className="text-[12px] text-[#3D5A55] font-medium mt-0.5 truncate">{location.state}</p>
        </div>
        <div className="min-w-0">
          <p className="text-[11px] font-black text-[#185d53] uppercase tracking-wider mb-1.5 flex items-center gap-1.5">
            <FiDollarSign className="text-[13px]" /> Avg Price
          </p>
          <p className="text-[15px] font-black text-[#00695C]">{formatCompact(location.avgPrice)}</p>
          <p className="text-[12px] text-[#3D5A55] font-medium mt-0.5">Total: {formatCompact(location.totalValue)}</p>
        </div>
        <div className="min-w-0">
          <p className="text-[11px] font-black text-[#185d53] uppercase tracking-wider mb-1.5 flex items-center gap-1.5">
            <FiCompass className="text-[13px]" /> Landmark
          </p>
          <p className="text-[13px] font-semibold text-[#1A2E2A] truncate">{location.landmark}</p>
          <p className="text-[12px] text-[#3D5A55] font-medium mt-0.5">Area: {location.areaSqkm} sq km</p>
        </div>
        <div className="min-w-0">
          <p className="text-[11px] font-black text-[#185d53] uppercase tracking-wider mb-1.5 flex items-center gap-1.5">
            <FiTrendingUp className="text-[13px]" /> Demand Score
          </p>
          <p className="text-[13px] font-semibold text-[#1A2E2A]">{location.demandScore}/100</p>
          <p className="text-[12px] text-[#3D5A55] font-medium mt-0.5">{location.inquiries} inquiries</p>
        </div>
      </div>

      <div className="grid grid-cols-3 gap-3 px-5 pb-4">
        <div className="flex items-center gap-2.5 px-4 py-2.5 bg-[#F5F9F8] rounded-xl border border-[#E8F0EE]">
          <FiHome className="text-[#00695C] text-base flex-shrink-0" />
          <div className="min-w-0">
            <p className="text-[15px] font-black text-[#0F1A18] leading-none">{location.totalProperties}</p>
            <p className="text-[10px] text-[#3D5A55] uppercase tracking-wider font-bold mt-1">Properties</p>
          </div>
        </div>
        <div className="flex items-center gap-2.5 px-4 py-2.5 bg-[#F5F9F8] rounded-xl border border-[#E8F0EE]">
          <FiCheckSquare className="text-[#10B981] text-base flex-shrink-0" />
          <div className="min-w-0">
            <p className="text-[15px] font-black text-[#00695C] leading-none">{location.activeProperties}</p>
            <p className="text-[10px] text-[#3D5A55] uppercase tracking-wider font-bold mt-1">Active</p>
          </div>
        </div>
        <div className="flex items-center gap-2.5 px-4 py-2.5 bg-[#F5F9F8] rounded-xl border border-[#E8F0EE]">
          <FiEye className="text-[#3B82F6] text-base flex-shrink-0" />
          <div className="min-w-0">
            <p className="text-[15px] font-black text-[#0F1A18] leading-none">{location.views}</p>
...
const LocationManagement = () => {
  ...
  useEffect(() => {
    ...
    ALL_CATEGORIES.forEach(cat => {
      const list = generateMockLocations(cat, Math.floor(Math.random() * 20) + 20, datePreset, customStart, customEnd);
      newLocations[cat] = list;
      newStats[cat] = buildLocationStats(cat, list, datePreset);
    });
  ...
  const currentConfig = LOCATION_CATEGORIES[activeCategory];
  ...
  const requestDeleteLocation = useCallback((locationId) => {
    const location = currentLocations.find(l => l.id === locationId);
    if (!location) return;
    setConfirmAction({
      title: 'Delete Location?',
      message: `Are you sure you want to delete "${location.locationName}" (${location.locationId})? This action cannot be undone.`,
      ...
      onConfirm: () => {
        setLocations(prev => {
          const updatedList = prev[activeCategory].filter(l => l.id !== locationId);
          const updated = { ...prev, [activeCategory]: updatedList };
          setCategoryStats(prevStats => ({
            ...prevStats,
            [activeCategory]: buildLocationStats(activeCategory, updatedList, datePreset)
          }));
          return updated;
        });
        setShowViewModal(false);
        setViewingLocation(null);
        showToast(`Location "${location.locationName}" deleted`, 'warning');
      }
    });
  }, [currentLocations, activeCategory, datePreset, showToast]);
  ...
  const handleRefresh = useCallback(() => {
    ...
    setTimeout(() => {
      const list = generateMockLocations(activeCategory, Math.floor(Math.random() * 20) + 20, datePreset, customStart, customEnd);
      setLocations(prev => ({ ...prev, [activeCategory]: list }));
      setCategoryStats(prev => ({ ...prev, [activeCategory]: buildLocationStats(activeCategory, list, datePreset) }));
  ...
  return (
    <div className="space-y-6 p-4 lg:p-6 min-h-screen bg-[#F8FAF9]">
      ...
      {showViewModal && viewingLocation && (
        <ViewLocationDetailModal
          location={viewingLocation}
          show={showViewModal}
          onClose={() => { setShowViewModal(false); setViewingLocation(null); }}
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-[#3D5A55] font-semibold flex items-center gap-2 flex-wrap">
              <span>Manage locations, zones and neighborhoods across all categories</span>
        ...
        <div className="relative z-0">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
            {ALL_CATEGORIES.map((cat, idx) => {
              const config = LOCATION_CATEGORIES[cat];
              const Icon = config.icon;
              const isActive = activeCategory === cat;
              const count = locations[cat]?.length || 0;
              return (
                <div
                  key={cat}
                ...
                >
                  ...
                  <div className={`flex items-center gap-2 pt-2 border-t ${isActive ? 'border-white/20' : 'border-[#E8F0EE]'}`}>
                    <FiMapPin className={`text-sm ${isActive ? 'text-white/90' : 'text-[#3D5A55]'}`} />
                    <span className={`text-xs font-bold ${isActive ? 'text-white' : 'text-[#0F1A18]'}`}>{count} locations</span>
                  </div>
                </div>
              );
            })}
        ...
        <>
          <div className="relative z-0 bg-white rounded-2xl p-2 border border-[#E8F0EE] shadow-sm flex items-center gap-1 overflow-x-auto">
            {ALL_CATEGORIES.map((cat) => {
              const Icon = LOCATION_CATEGORIES[cat].icon;
              const isActive = activeCategory === cat;
              return (
                <button
                  key={cat}
                  ...
                  className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-bold whitespace-nowrap transition-all ${
                    isActive
                      ? `bg-gradient-to-r ${LOCATION_CATEGORIES[cat].gradient} text-white shadow-md`
