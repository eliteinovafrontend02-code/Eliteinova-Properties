// src/components/admin/Payments/TransactionActions.jsx
...
import {
  ...
  FiClipboard, FiLayers, FiMapPin, FiFileText, FiFile,
  FiCheckSquare, FiThumbsUp, FiSend, FiUserCheck, FiTrendingUp,
  FiPaperclip, FiMessageSquare, FiExternalLink, FiRepeat,
  FiShield, FiUnlock, FiLock, FiArchive, FiInbox, FiStar,
  FiAward, FiZap, FiTool, FiSettings, FiHelpCircle, FiPhone, FiMail,
  FiGlobe, FiUsers, FiPieChart
} from 'react-icons/fi';
import { FaHome, FaHotel, FaHardHat, FaRupeeSign } from 'react-icons/fa';
...
const ALL_PAYMENT_RELATED_TO = [
  'Property Listing Fee', 'Property Promotion', 'Featured Listing', 'Subscription',
  'Lead Purchase', 'Contact Access', 'Agent Subscription', 'Builder Subscription',
  'Owner Subscription', 'Property Management Service', 'Loan Service'
];
...
// ============================================================
// VERIFICATION / APPROVAL STATUS CONFIG
// ============================================================
const VERIFICATION_STATUS = {
  'Unverified': { icon: FiShield,      bg: 'bg-gray-50',   text: 'text-gray-700',   border: 'border-gray-200',   label: 'Unverified' },
  'Verified':   { icon: FiCheckSquare, bg: 'bg-teal-50',   text: 'text-teal-700',   border: 'border-teal-200',   label: 'Verified' },
  'Rejected':   { icon: FiXCircle,     bg: 'bg-red-50',    text: 'text-red-700',    border: 'border-red-200',    label: 'Rejected' }
};

const APPROVAL_STATUS = {
  'Pending Approval': { icon: FiClock,       bg: 'bg-amber-50',   text: 'text-amber-700',   border: 'border-amber-200',   label: 'Pending Approval' },
  'Approved':         { icon: FiThumbsUp,    bg: 'bg-emerald-50', text: 'text-emerald-700', border: 'border-emerald-200', label: 'Approved' },
  'Declined':         { icon: FiXCircle,     bg: 'bg-red-50',     text: 'text-red-700',     border: 'border-red-200',     label: 'Declined' }
};
...
// ============================================================
const ADMIN_ACTIONS = [
  { key: 'view',                label: 'View Payment',            icon: FiEye,          color: 'text-[#00695C]', bg: 'bg-[#E8F4F2]', hover: 'hover:bg-[#C5EDE5]', group: 'primary' },
  { key: 'verify',              label: 'Verify Payment',          icon: FiShield,       color: 'text-teal-700',  bg: 'bg-teal-50',   hover: 'hover:bg-teal-100',  group: 'verification' },
  { key: 'approve',             label: 'Approve Payment',         icon: FiThumbsUp,     color: 'text-emerald-700', bg: 'bg-emerald-50', hover: 'hover:bg-emerald-100', group: 'approval' },
  { key: 'markPaid',            label: 'Mark as Paid',            icon: FiCheckSquare,  color: 'text-green-700',  bg: 'bg-green-50',  hover: 'hover:bg-green-100',  group: 'payment' },
  { key: 'refund',              label: 'Process Refund',          icon: FiRotateCcw,    color: 'text-blue-700',   bg: 'bg-blue-50',   hover: 'hover:bg-blue-100',   group: 'payment' },
  { key: 'downloadReceipt',     label: 'Download Receipt',        icon: FiDownload,     color: 'text-indigo-700', bg: 'bg-indigo-50', hover: 'hover:bg-indigo-100', group: 'download' },
  { key: 'downloadInvoice',     label: 'Download Invoice',        icon: FiFileText,     color: 'text-violet-700', bg: 'bg-violet-50', hover: 'hover:bg-violet-100', group: 'download' },
  { key: 'addNote',             label: 'Add Payment Note',        icon: FiMessageSquare, color: 'text-amber-700', bg: 'bg-amber-50',  hover: 'hover:bg-amber-100',  group: 'note' },
  { key: 'viewCustomer',        label: 'View Customer',           icon: FiUserCheck,    color: 'text-pink-700',   bg: 'bg-pink-50',   hover: 'hover:bg-pink-100',   group: 'view' },
  { key: 'viewProperty',        label: 'View Property',           icon: FiHome,         color: 'text-cyan-700',   bg: 'bg-cyan-50',   hover: 'hover:bg-cyan-100',   group: 'view' },
  { key: 'viewTransaction',     label: 'View Transaction History', icon: FiRepeat,      color: 'text-slate-700',  bg: 'bg-slate-50',  hover: 'hover:bg-slate-100',  group: 'view' }
];
...
// ============================================================
// ✅ FIXED: VIEW PROPERTY PAYMENT DETAIL MODAL
// All quick actions now properly close the view modal first
// ============================================================
const ViewPropertyPaymentDetailModal = ({ payment, show, onClose, onEdit, onDelete, onAction }) => {
  ...
  // ✅ Helper: close view modal then trigger action
  const triggerAction = (actionKey) => {
    onClose(); // close view modal FIRST
    setTimeout(() => {
      if (onAction) onAction(actionKey, payment);
    }, 150); // slight delay so close animation plays
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
      <div className="bg-white rounded-3xl max-w-4xl w-full max-h-[90vh] overflow-hidden shadow-2xl animate-slide-up border border-[#E8F0EE] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-white">
          ...
          {/* Quick Admin Actions inside View Modal */}
          <div className="mt-6">
            <h3 className="text-xs font-semibold text-[#5A7D78] uppercase tracking-wider mb-3 flex items-center gap-2">
              <FiZap className="text-[#00695C]" /> Quick Actions
            </h3>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              {ADMIN_ACTIONS.filter(a => a.key !== 'view').map(action => (
                <button
                  key={action.key}
                  onClick={() => triggerAction(action.key)}
                  className={`flex items-center gap-2 px-3 py-2.5 rounded-xl text-xs font-semibold transition-all duration-300 hover:scale-105 ${action.bg} ${action.color} ${action.hover}`}
                >
        ...
        <div className="sticky bottom-0 px-6 py-4 bg-white border-t border-[#E8F0EE] rounded-b-3xl shrink-0 shadow-[0_-4px_20px_rgba(0,0,0,0.05)]">
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={() => { onClose(); setTimeout(() => onEdit && onEdit(payment), 150); }}
            ...
            <button
              onClick={() => { onClose(); setTimeout(() => onDelete && onDelete(payment.id), 150); }}
...
// ============================================================
// ✅ NEW: VIEW PROPERTY MODAL
// Opens when "View Property" action is clicked
// ============================================================
const ViewPropertyModal = ({ payment, show, onClose }) => {
  if (!payment || !show) return null;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
      <div className="bg-white rounded-3xl max-w-2xl w-full max-h-[90vh] shadow-2xl animate-slide-up border border-[#E8F0EE] overflow-hidden flex flex-col">
        {/* Header */}
        <div className="bg-gradient-to-r from-cyan-700 to-cyan-500 p-5 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-white/80 text-xs">{payment.propertyId} — {payment.propertyName}</p>
            </div>
          </div>
          <button onClick={onClose} className="w-8 h-8 rounded-full bg-white/20 hover:bg-white/30 flex items-center justify-center text-white transition-all hover:scale-110">
            <FiX className="text-lg" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          {/* Property Hero */}
          <div className="flex items-center gap-4 p-4 bg-[#F5F9F8] rounded-2xl">
            <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${propTypeConfig.gradient} flex items-center justify-center text-white shadow-lg`}>
              <PropTypeIcon className="text-2xl" />
            </div>
            <div className="min-w-0 flex-1">
              <h4 className="text-lg font-bold text-[#1A2E2A] truncate">{payment.propertyName}</h4>
              <div className="flex items-center gap-2 mt-1 flex-wrap">
                <span className={`text-[11px] px-2 py-0.5 rounded-full font-semibold ${propTypeConfig.bg} ${propTypeConfig.text} border ${propTypeConfig.border}`}>
                  {payment.propertyType}
                </span>
                <span className={`text-[11px] px-2 py-0.5 rounded-full font-semibold ${statusConfig.bg} ${statusConfig.text} border ${statusConfig.border}`}>
                  {statusConfig.label}
                </span>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="bg-[#F5F9F8] rounded-xl p-3">
              <div className="flex items-center gap-2 mb-1">
                <FiHash className="text-[#00695C] text-xs" />
                <span className="text-[10px] font-semibold text-[#5A7D78] uppercase tracking-wider">Property ID</span>
              </div>
              <p className="text-sm font-bold text-[#1A2E2A]">{payment.propertyId}</p>
            </div>
            ...
            <div className="bg-[#F5F9F8] rounded-xl p-3">
              <div className="flex items-center gap-2 mb-1">
                <FiTag className="text-[#00695C] text-xs" />
                <span className="text-[10px] font-semibold text-[#5A7D78] uppercase tracking-wider">Purpose</span>
              </div>
              <p className="text-sm font-bold text-[#1A2E2A]">{payment.propertyPurpose}</p>
            </div>

            <div className="bg-[#F5F9F8] rounded-xl p-3 sm:col-span-2">
              <div className="flex items-center gap-2 mb-1">
                <FiMapPin className="text-[#00695C] text-xs" />
                <span className="text-[10px] font-semibold text-[#5A7D78] uppercase tracking-wider">Location</span>
              </div>
              <p className="text-sm font-bold text-[#1A2E2A]">{payment.propertyLocation}</p>
            </div>
            ...
            <div className="bg-[#F5F9F8] rounded-xl p-3">
              <div className="flex items-center gap-2 mb-1">
                <RoleIcon className="text-[#00695C] text-xs" />
                <span className="text-[10px] font-semibold text-[#5A7D78] uppercase tracking-wider">Listing {payment.listingRole}</span>
              </div>
              <p className="text-sm font-bold text-[#1A2E2A]">{payment.listingName}</p>
            </div>
            ...
            <div className="bg-[#F5F9F8] rounded-xl p-3">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-[10px] font-semibold text-[#5A7D78] uppercase tracking-wider">Payment Related To</span>
              </div>
              <p className="text-sm font-bold text-[#1A2E2A]">{payment.paymentRelatedTo}</p>
            </div>
          </div>
          ...
          {payment.note && (
            <div className="bg-amber-50 rounded-xl p-3 border border-amber-200">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-[10px] font-semibold text-amber-700 uppercase tracking-wider">Payment Note</span>
              </div>
              <p className="text-sm text-amber-800">{payment.note}</p>
            </div>
          )}
        </div>

        {/* Footer */}
        <div className="px-5 py-4 bg-white border-t border-[#E8F0EE] shrink-0">
...
// ADD NOTE MODAL
// ============================================================
const AddNoteModal = ({ payment, show, onClose, onSave }) => {
  const [note, setNote] = useState('');
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    if (payment) setNote(payment.note || '');
  }, [payment]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
      <div className="bg-white rounded-3xl max-w-lg w-full shadow-2xl animate-slide-up border border-[#E8F0EE] overflow-hidden">
        <div className="bg-gradient-to-r from-[#00695C] to-[#26A69A] p-5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-white/20 flex items-center justify-center text-white">
              <FiMessageSquare className="text-lg" />
            </div>
            <div>
              <h3 className="text-lg font-bold text-white">Add Payment Note</h3>
              <p className="text-white/80 text-xs">{payment.propertyName} — {payment.propertyId}</p>
            </div>
          </div>
          <button onClick={onClose} className="w-8 h-8 rounded-full bg-white/20 hover:bg-white/30 flex items-center justify-center text-white transition-all hover:scale-110">
            <FiX className="text-lg" />
        ...
        <form onSubmit={handleSubmit} className="p-5">
          <textarea
            ...
            className="w-full px-4 py-3 bg-[#F5F9F8] rounded-xl border border-[#E8F0EE] focus:border-[#00695C] focus:ring-2 focus:ring-[#00695C]/20 transition-all duration-300 text-sm text-[#1A2E2A] outline-none resize-none placeholder:text-[#B5C9C5]"
          />
          <div className="flex items-center gap-3 mt-4">
            ...
            <button type="submit" disabled={loading || !note.trim()} className="flex-1 px-4 py-2.5 bg-[#00695C] text-white rounded-xl hover:bg-[#004D40] transition-all duration-300 text-sm font-medium shadow-lg shadow-[#00695C]/30 hover:scale-[1.02] disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
...
// ============================================================
const TransactionHistoryModal = ({ payment, history, show, onClose }) => {
  if (!payment || !show) return null;

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
      <div className="bg-white rounded-3xl max-w-2xl w-full max-h-[85vh] shadow-2xl animate-slide-up border border-[#E8F0EE] overflow-hidden flex flex-col">
        <div className="bg-gradient-to-r from-[#00695C] to-[#26A69A] p-5 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-white/20 flex items-center justify-center text-white">
        ...
        <div className="flex-1 overflow-y-auto p-5">
          {(!history || history.length === 0) ? (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              <div className="w-20 h-20 rounded-full bg-[#F5F9F8] flex items-center justify-center mb-3">
                <FiInbox className="text-3xl text-[#B5C9C5]" />
              </div>
              <h4 className="text-base font-bold text-[#1A2E2A]">No history available</h4>
              <p className="text-xs text-[#5A7D78] mt-1">This payment has no recorded transactions yet.</p>
            </div>
          ) : (
            <div className="relative">
              <div className="absolute left-[18px] top-2 bottom-2 w-0.5 bg-[#E8F0EE]" />
              <div className="space-y-4">
                {history.map((item, idx) => (
                  <div key={idx} className="flex items-start gap-4 relative">
                    <div className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 z-10 border-2 border-white shadow-md ${item.bg || 'bg-[#E8F4F2]'} ${item.color || 'text-[#00695C]'}`}>
                      <item.icon className="text-sm" />
                    </div>
                    <div className="flex-1 bg-[#F5F9F8] rounded-xl p-3 min-w-0">
                      <div className="flex items-center justify-between gap-2 flex-wrap">
                        <p className="text-sm font-bold text-[#1A2E2A]">{item.title}</p>
                        <span className="text-[10px] text-[#5A7D78] font-medium shrink-0">{item.time}</span>
                      </div>
                      <p className="text-xs text-[#5A7D78] mt-0.5">{item.description}</p>
                      {item.amount != null && (
                        <p className="text-xs font-bold text-[#00695C] mt-1">{formatCurrency(item.amount)}</p>
                      )}
                    </div>
                  </div>
                ))}
...
// ============================================================
const ViewCustomerModal = ({ payment, show, onClose }) => {
  if (!payment || !show) return null;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
      <div className="bg-white rounded-3xl max-w-lg w-full shadow-2xl animate-slide-up border border-[#E8F0EE] overflow-hidden flex flex-col max-h-[90vh]">
        <div className="bg-gradient-to-r from-pink-600 to-pink-400 p-5 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-white/20 flex items-center justify-center text-white">
            ...
            <div>
              ...
              <p className="text-white/80 text-xs">{payment.listingRole} — {payment.listingName}</p>
            </div>
          </div>
          <button onClick={onClose} className="w-8 h-8 rounded-full bg-white/20 hover:bg-white/30 flex items-center justify-center text-white transition-all hover:scale-110">
            <FiX className="text-lg" />
        ...
        <div className="p-5 space-y-3 overflow-y-auto">
          <div className="flex items-center gap-3 p-4 bg-[#F5F9F8] rounded-2xl">
            <div className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${roleConfig.gradient} flex items-center justify-center text-white shadow-lg`}>
              <RoleIcon className="text-2xl" />
            </div>
            <div className="min-w-0">
              <p className="text-lg font-bold text-[#1A2E2A] truncate">{payment.listingName}</p>
              <span className={`inline-block text-[11px] px-2 py-0.5 rounded-full font-semibold ${roleConfig.bg} ${roleConfig.text} border ${roleConfig.border}`}>
                {payment.listingRole}
              </span>
            </div>
          </div>
          <div className="grid grid-cols-1 gap-2">
            <div className="flex items-center justify-between p-3 bg-[#F5F9F8] rounded-xl">
              <span className="text-xs font-semibold text-[#5A7D78] uppercase tracking-wider">Customer ID</span>
              <span className="text-sm font-bold text-[#1A2E2A]">CUST-{String(payment.id || '').replace(/\D/g, '').slice(-4).padStart(4, '0')}</span>
            </div>
            <div className="flex items-center justify-between p-3 bg-[#F5F9F8] rounded-xl">
              <span className="text-xs font-semibold text-[#5A7D78] uppercase tracking-wider">Linked Property</span>
              <span className="text-sm font-bold text-[#1A2E2A] truncate ml-3">{payment.propertyName}</span>
            </div>
            <div className="flex items-center justify-between p-3 bg-[#F5F9F8] rounded-xl">
              <span className="text-xs font-semibold text-[#5A7D78] uppercase tracking-wider">Payment Status</span>
              <span className={`text-[11px] px-2 py-0.5 rounded-full font-semibold ${statusConfig.bg} ${statusConfig.text} border ${statusConfig.border}`}>
                {statusConfig.label}
              </span>
            </div>
            <div className="flex items-center justify-between p-3 bg-[#F5F9F8] rounded-xl">
              <span className="text-xs font-semibold text-[#5A7D78] uppercase tracking-wider">Total Paid</span>
              <span className="text-sm font-bold text-[#00695C]">{formatCurrency(payment.amount)}</span>
            </div>
            <div className="flex items-center justify-between p-3 bg-[#F5F9F8] rounded-xl">
              <span className="text-xs font-semibold text-[#5A7D78] uppercase tracking-wider">Payment For</span>
              <span className="text-sm font-bold text-[#1A2E2A] truncate ml-3">{payment.paymentRelatedTo}</span>
            </div>
          </div>
        </div>
        <div className="px-5 py-4 bg-white border-t border-[#E8F0EE] shrink-0">
...
// ============================================================
const TransactionActions = ({ payments: externalPayments, onPaymentsChange }) => {
  ...
  const generateMockPayments = useCallback(() => {
    ...
    for (let i = 1; i <= 80; i++) {
      ...
      const verification = ['Unverified', 'Verified', 'Rejected'][Math.floor(Math.random() * 3)];
      const approval = ['Pending Approval', 'Approved', 'Declined'][Math.floor(Math.random() * 3)];
  ...
  // ============ INITIALIZE DATA ============
  useEffect(() => {
    if (externalPayments && externalPayments.length > 0) {
      setPayments(externalPayments);
      setFilteredPayments(externalPayments);
      computeStats(externalPayments);
    } else {
      try {
  ...
  }, [externalPayments, generateMockPayments, computeStats]);
  ...
  // ============ ✅ FIXED: ADMIN ACTION HANDLER ============
  // All actions now properly open their respective modals
  const handleAdminAction = useCallback((actionKey, payment) => {
    switch (actionKey) {
      case 'view':
      ...
      case 'verify':
        setConfirmationModal({
          ...
          message: `Mark payment for "${payment.propertyName}" as verified?`,
          confirmText: 'Verify',
          cancelText: 'Cancel',
          type: 'info',
          onConfirm: () => {
            setPayments(prev => {
              const updated = prev.map(p => p.id === payment.id ? { ...p, verification: 'Verified' } : p);
              computeStats(updated);
              return updated;
            });
            setToast({ message: `Payment "${payment.propertyId}" verified successfully`, type: 'success' });
          },
          onCancel: () => setConfirmationModal(prev => ({ ...prev, isOpen: false }))
        });
        break;
      case 'approve':
        setConfirmationModal({
          ...
          message: `Approve payment for "${payment.propertyName}"?`,
          confirmText: 'Approve',
          cancelText: 'Cancel',
          type: 'info',
          onConfirm: () => {
            setPayments(prev => {
              const updated = prev.map(p => p.id === payment.id ? { ...p, approval: 'Approved' } : p);
              computeStats(updated);
              return updated;
            });
            setToast({ message: `Payment "${payment.propertyId}" approved successfully`, type: 'success' });
          },
          onCancel: () => setConfirmationModal(prev => ({ ...prev, isOpen: false }))
        });
        break;
      case 'markPaid':
        setConfirmationModal({
          isOpen: true,
          title: 'Mark as Paid',
          message: `Mark payment for "${payment.propertyName}" as paid?`,
          confirmText: 'Mark Paid',
          cancelText: 'Cancel',
          type: 'info',
          onConfirm: () => {
            setPayments(prev => {
              const updated = prev.map(p => p.id === payment.id ? { ...p, status: 'Success' } : p);
              computeStats(updated);
              return updated;
            });
            setToast({ message: `Payment "${payment.propertyId}" marked as paid`, type: 'success' });
          },
          onCancel: () => setConfirmationModal(prev => ({ ...prev, isOpen: false }))
        });
        break;
      case 'refund':
        setConfirmationModal({
          ...
          message: `Process refund of ${formatCurrency(payment.amount)} for "${payment.propertyName}"?`,
          confirmText: 'Refund',
          cancelText: 'Cancel',
          type: 'warning',
          onConfirm: () => {
            setPayments(prev => {
              const updated = prev.map(p => p.id === payment.id ? { ...p, status: 'Refunded' } : p);
              computeStats(updated);
              return updated;
            });
            setToast({ message: `Refund processed for "${payment.propertyId}"`, type: 'success' });
          },
          onCancel: () => setConfirmationModal(prev => ({ ...prev, isOpen: false }))
        });
        break;
      case 'downloadReceipt':
        setToast({ message: `Downloading receipt for "${payment.propertyId}"...`, type: 'info' });
        setTimeout(() => setToast({ message: `Receipt for "${payment.propertyId}" downloaded`, type: 'success' }), 1200);
        break;
      case 'downloadInvoice':
        setToast({ message: `Downloading invoice for "${payment.propertyId}"...`, type: 'info' });
        setTimeout(() => setToast({ message: `Invoice for "${payment.propertyId}" downloaded`, type: 'success' }), 1200);
        break;
      case 'addNote':
      ...
      case 'viewCustomer':
        ...
        break;
      case 'viewProperty':
        // ✅ FIXED: Now opens a proper Property Modal
        setPropertyPayment(payment);
  ...
  const handleSaveNote = useCallback((paymentId, note) => {
    setPayments(prev => prev.map(p => p.id === paymentId ? { ...p, note } : p));
    setToast({ message: 'Payment note saved successfully', type: 'success' });
  }, []);

  // ============ TRANSACTION HISTORY BUILDER ============
  const buildHistory = useCallback((payment) => {
    if (!payment) return [];
    const items = [
      {
        icon: FiCreditCard,
        title: 'Payment Initiated',
        description: `${payment.paymentRelatedTo} for ${payment.propertyName}`,
        time: new Date(payment.paymentDate).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' }),
    ...
    if (payment.verification === 'Verified') {
      items.push({
        ...
        description: 'Payment details verified by admin',
        time: 'After 2 hours',
        bg: 'bg-teal-50',
        color: 'text-teal-600'
      });
    }
    if (payment.approval === 'Approved') {
      items.push({
        icon: FiThumbsUp,
    ...
    if (payment.status === 'Refunded') {
      items.push({
        ...
        description: 'Full amount refunded to customer',
        time: 'After 2 days',
        amount: payment.amount,
        bg: 'bg-blue-50',
        color: 'text-blue-600'
    ...
    <div className="space-y-6 p-4 lg:p-6 bg-[#F8FAF9] min-h-screen">
      ...
      {showNoteModal && notePayment && (
        <AddNoteModal
          payment={notePayment}
          show={showNoteModal}
          onClose={() => { setShowNoteModal(false); setNotePayment(null); }}
      ...
      {showHistoryModal && historyPayment && (
        <TransactionHistoryModal
          ...
          history={buildHistory(historyPayment)}
          show={showHistoryModal}
          onClose={() => { setShowHistoryModal(false); setHistoryPayment(null); }}
        />
      )}

      {showCustomerModal && customerPayment && (
        <ViewCustomerModal
          payment={customerPayment}
          show={showCustomerModal}
          onClose={() => { setShowCustomerModal(false); setCustomerPayment(null); }}
        />
      )}

      {/* ✅ NEW: View Property Modal */}
      {showPropertyModal && propertyPayment && (
        <ViewPropertyModal
          payment={propertyPayment}
          show={showPropertyModal}
          onClose={() => { setShowPropertyModal(false); setPropertyPayment(null); }}
        />
      )}
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-[#5A7D78] flex items-center gap-2 flex-wrap">
              <span>Verify, approve, refund and manage all property-linked payments from a single place</span>
      ...
      <div className="relative bg-white rounded-2xl p-4 shadow-sm border border-[#E8F0EE] hover:shadow-md transition-all duration-300">
        ...
        <div className="flex items-center gap-2 flex-wrap mt-5">
          <FilterDropdown label="Type" options={propertyTypeOptions} value={activePropertyType} onChange={setActivePropertyType} icon={FiHome} allLabel="All Types" />
          <FilterDropdown label="Purpose" options={purposeOptions} value={activePurpose} onChange={setActivePurpose} icon={FiTag} allLabel="All Purposes" />
          <FilterDropdown label="Related To" options={relatedToOptions} value={activeRelatedTo} onChange={setActiveRelatedTo} icon={FiCreditCard} allLabel="All Payment Types" />
          <FilterDropdown label="Listing" options={listingRoleOptions} value={activeListingRole} onChange={setActiveListingRole} icon={FiUser} allLabel="All Roles" />
            ...
            {paginatedPayments.map((payment, index) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-2 pt-2 border-t border-[#E8F0EE]">
                    <p className="text-[9px] font-bold text-[#5A7D78] uppercase tracking-wider mb-1.5">Admin Actions</p>
                    <div className="grid grid-cols-4 gap-1">
                      {ADMIN_ACTIONS.filter(a => !['view', 'downloadReceipt', 'downloadInvoice', 'addNote', 'viewCustomer', 'viewProperty', 'viewTransaction'].includes(a.key)).map(action => (
                        <button
                          key={action.key}
                          type="button"
                          onClick={() => handleAdminAction(action.key, payment)}
                          className={`py-1.5 text-[9px] font-semibold rounded-lg transition-all duration-300 flex flex-col items-center gap-0.5 hover:scale-105 ${action.bg} ${action.color} ${action.hover}`}
                          title={action.label}
                        >
                          <action.icon className="text-[11px]" />
                        </button>
                      ))}
                      <button
                        type="button"
                        onClick={() => handleAdminAction('downloadReceipt', payment)}
                        className="py-1.5 text-[9px] font-semibold rounded-lg transition-all duration-300 flex flex-col items-center gap-0.5 hover:scale-105 bg-indigo-50 text-indigo-700 hover:bg-indigo-100"
                      ...
                      <button
                        type="button"
                        onClick={() => handleAdminAction('downloadInvoice', payment)}
                        className="py-1.5 text-[9px] font-semibold rounded-lg transition-all duration-300 flex flex-col items-center gap-0.5 hover:scale-105 bg-violet-50 text-violet-700 hover:bg-violet-100"
                      ...
                      <button
                        type="button"
                        onClick={() => handleAdminAction('addNote', payment)}
                        className="py-1.5 text-[9px] font-semibold rounded-lg transition-all duration-300 flex flex-col items-center gap-0.5 hover:scale-105 bg-amber-50 text-amber-700 hover:bg-amber-100"
                        title="Add Note"
                      >
                      ...
                      <button
                        type="button"
                        onClick={() => handleAdminAction('viewTransaction', payment)}
                        className="py-1.5 text-[9px] font-semibold rounded-lg transition-all duration-300 flex flex-col items-center gap-0.5 hover:scale-105 bg-slate-50 text-slate-700 hover:bg-slate-100"
            ...
            {paginatedPayments.map((payment, index) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="col-span-1 min-w-0 text-xs font-medium text-[#5A7D78] truncate">{payment.propertyType}</div>
                  <div className="col-span-1 min-w-0 text-xs font-medium text-[#5A7D78] truncate">{payment.propertyPurpose}</div>
                  ...
                  <div className="col-span-1 min-w-0 text-xs font-medium text-[#5A7D78] truncate">{payment.paymentRelatedTo}</div>
