...
import { FaCity, FaCrown, FaHome, FaBuilding, FaHotel } from 'react-icons/fa';
...
// ============================================================
const ViewOwnerDetailModal = ({ owner, show, onClose, onEdit, onDelete }) => {
  if (!owner || !show) return null;

  const planConfig = PLAN_TYPES[owner.subscriptionPlan] || PLAN_TYPES['Basic'];
      ...
      <div className="bg-white rounded-3xl max-w-4xl w-full max-h-[90vh] overflow-hidden shadow-2xl animate-slide-up border border-[#E8F0EE] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-white">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {/* ===== Location fields: Street → Area/Locality → City → District → State → Country → Pincode ===== */}
            <div className="bg-[#F5F9F8] rounded-2xl p-4">
              <div className="flex items-center gap-2 mb-1">
        ...
        <div className="sticky bottom-0 px-6 py-4 bg-white border-t border-[#E8F0EE] rounded-b-3xl shrink-0 shadow-[0_-4px_20px_rgba(0,0,0,0.05)]">
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={() => { if (onEdit) { onEdit(owner); onClose(); } }}
            ...
            <button
              onClick={() => { if (onDelete) { onDelete(owner.id); } }}
            ...
            <div className="bg-[#F5F9F8] rounded-2xl p-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    type="text" name="ownerId" value={formData.ownerId} onChange={handleChange}
...
const AgentPlans = () => {
  ...
  // ============ STATS (total + one count per plan) ============
  const [stats, setStats] = useState({ total: 0, Basic: 0, Professional: 0, Enterprise: 0 });
  ...
  const generateMockOwners = useCallback(() => {
    ...
    for (let i = 1; i <= 60; i++) {
      ...
      const phoneNum = `+91 ${String(70000 + Math.floor(Math.random() * 29999)).padStart(5, '9')} ${String(10000 + Math.floor(Math.random() * 89999)).slice(0, 5)}`;

      list.push({
        id: `agt_${i}`,
        ownerId: `AGT-${String(i).padStart(4, '0')}`,
        name,
    ...
    try {
      ...
      if (searchQuery) {
        ...
        filtered = filtered.filter(o =>
          ...
          (o.street && o.street.toLowerCase().includes(query)) ||
          (o.pincode && o.pincode.toLowerCase().includes(query))
        );
      }

      if (activePlan !== 'all') {
        filtered = filtered.filter(o => o.subscriptionPlan === activePlan);
      }

      if (activePropertyType !== 'all') {
        filtered = filtered.filter(o => o.propertyType === activePropertyType);
      }
  ...
  const paginatedOwners = useMemo(() => {
    ...
    return filteredOwners.slice(start, end);
  ...
  const handleSelectAll = useCallback(() => {
    if (selectedOwners.length === paginatedOwners.length && paginatedOwners.length > 0) {
      setSelectedOwners([]);
    } else {
  ...
  const handleSaveOwner = useCallback((updatedOwner) => {
    ...
    setToast({ message: `Agent "${updatedOwner.name}" updated successfully`, type: 'success' });
  }, [computeStats]);
  ...
  const handleDeleteOwner = useCallback((ownerId) => {
    ...
    setConfirmationModal({
      isOpen: true,
      title: 'Delete Agent',
      message: `Are you sure you want to delete agent "${owner.name}" (${owner.ownerId})?`,
      confirmText: 'Delete',
      cancelText: 'Cancel',
      type: 'danger',
      onConfirm: () => {
        ...
        setTimeout(() => {
          ...
          setToast({ message: `Deleted agent "${owner.name}"`, type: 'warning' });
        }, 700);
      },
  ...
  const handleExport = useCallback(() => {
    ...
    try {
      const data = filteredOwners.map(o => ({
        ...
        'Subscription Plan': (PLAN_TYPES[o.subscriptionPlan]?.label) || o.subscriptionPlan || '',
        'Street': o.street || '',
        'Area / Locality': o.area || '',
        'City': o.city || '',
        'District': o.district || '',
        'State': o.state || '',
        'Country': o.country || '',
  ...
  const handleBulkDelete = useCallback(() => {
    ...
    setConfirmationModal({
      ...
      message: `Are you sure you want to delete ${selectedOwners.length} selected agent(s)?`,
  ...
  }, [selectedOwners, owners, computeStats]);
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-[#5A7D78] flex items-center gap-2 flex-wrap">
              <span>Basic, Professional &amp; Enterprise agent subscriptions</span>
        ...
        {selectedOwners.length > 0 && (
          <div className="mt-4 pt-4 border-t border-[#E8F0EE] flex flex-wrap items-center justify-between gap-3 animate-slide-in">
            <span className="text-sm text-[#5A7D78]">
              <span className="font-semibold text-[#00695C]">{selectedOwners.length}</span> agent(s) selected
            </span>
            ...
            {paginatedOwners.map((owner, index) => {
              ...
              return (
                ...
                >
                  ...
                  {/* ===== Contact + property + location fields ===== */}
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-2 text-[11px] text-[#5A7D78]">
                      ...
                      <span className="truncate font-medium">{owner.district}, {owner.state}</span>
                    </div>
                    <div className="flex items-center gap-2 text-[11px] text-[#5A7D78]">
                      <FiHash className="text-[#00695C] flex-shrink-0" />
                      <span className="truncate font-semibold text-[#1A2E2A]">{owner.pincode}</span>
                    </div>
                  </div>
                  {/* ===== End fields ===== */}
